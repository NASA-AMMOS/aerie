...
public class LockModel<TaskId> {
  ...
  AutoRelease lock(final TaskId taskId) {
    System.out.println(taskId + " waiting on lock");
    if (queue.contains(taskId)) {
      throw new IllegalStateException("Cannot deterministically order duplicate taskId: " + taskId);
    }
    queue.add(taskId);
    waitUntil((positive, atEarliest, atLatest) -> {
      if (positive == (this.queue.peek() == taskId)) {
        System.out.println(taskId + " claimed lock");
        return Optional.of(atEarliest);
      } else {
        return Optional.empty();
      }
    });
    return () -> LockModel.this.release(taskId);
  }
  ...
  void release(final TaskId taskId) {
    ...
    System.out.println("Lock released by " + taskId);
    queue.remove();
  }
