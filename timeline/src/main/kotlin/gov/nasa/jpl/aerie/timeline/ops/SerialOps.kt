 ...
 */
interface SerialOps<V : Any, THIS: SerialOps<V, THIS>>: SegmentOps<V, THIS> {
  ...
  infix fun <OTHER: SerialOps<V, OTHER>> set(other: SerialOps<V, OTHER>) = map2Values(other, BinaryOperation.combineOrIdentity { _, r, _ -> r })

  /**
   * [(DOC)][map2Values] Performs a local binary operation between two profiles where the result
   * is the same type as this profile.
   */
  fun <W: Any, OTHER: SerialOps<W, OTHER>> map2Values(other: SerialOps<W, OTHER>, op: BinaryOperation<V, W, V?>) = map2Values(ctor, other, op)

  /**
   * [(DOC)][map2Values] Performs a local binary operation between two profiles.
   *
   * The operation will be evaluated on each pair of segments that overlap, with their
   * intersection supplied as the interval argument to the [BinaryOperation]. The result of
   ...
   * argument in [BinaryOperation]. The operation is not called for intervals that have a gap
   ...
   */
  fun <W: Any, OTHER: SerialOps<W, OTHER>, R: Any, RESULT: GeneralOps<Segment<R>, RESULT>> map2Values(ctor: (Timeline<Segment<R>, RESULT>) -> RESULT, other: SerialOps<W, OTHER>, op: BinaryOperation<V, W, R?>) =
      unsafeOperate(ctor) { bounds -> map2SegmentLists(collect(bounds), other.collect(bounds), op) }

  /**
   * [(DOC)][flatMap2Values] Performs a local binary operation that produces profiles, and flattens
   * it into a profile of the same type as this.
   */
  fun <W: Any, OTHER: SerialOps<W, OTHER>, NESTED: SerialOps<V, NESTED>> flatMap2Values(other: SerialOps<W, OTHER>, op: BinaryOperation<V, W, NESTED?>) = flatMap2Values(ctor, other, op)

  /**
   * [(DOC)][flatMap2Values] Performs a local binary operation that produces profiles, and flattens it.
   *
   * Similar to [map2Values], except it expects the [BinaryOperation] to return a profile. Each nested profile
   ...
   */
  fun detectEdges(edgePredicate: BinaryOperation<V, V, Boolean?>) = unsafeOperate(::Booleans) { opts ->
