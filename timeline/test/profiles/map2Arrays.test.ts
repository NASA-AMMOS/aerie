...
test("basic combine or identity", () => {
  ...
  const result = map2Arrays(left, right, BinaryOperation.combineOrIdentity((l, r) => l + r));
...
test("basic combine or undefined", () => {
  ...
  const result = map2Arrays(left, right, BinaryOperation.combineOrUndefined((l, r) => l + r));
...
describe("map2Arrays segment alignments", () => {
  ...
  const makeLeft = (s: number, e: number, si: Inclusivity = Inclusivity.Inclusive, ei: Inclusivity = Inclusivity.Inclusive) => [new Segment(-1, between(dur(s), dur(e), si, ei))];
  const makeRight = (s: number, e: number, si: Inclusivity = Inclusivity.Inclusive, ei: Inclusivity = Inclusivity.Inclusive) => [new Segment(1, between(dur(s), dur(e), si, ei))];
  ...
  test("identical", () => {
    expect(map2Arrays(
        ...
        makeRight(1, 2),
        op
    )).toEqual([new Segment(0, between(dur(1), dur(2)))]);
  });
  ...
  test("left first, moment of non-overlap", () => {
    expect(map2Arrays(
        makeLeft(1, 2),
        makeRight(1, 2, Inclusivity.Exclusive, Inclusivity.Inclusive),
        op
    )).toEqual([
  ...
  test("right first, moment of non-overlap", () => {
    expect(map2Arrays(
        makeLeft(1, 2, Inclusivity.Exclusive, Inclusivity.Inclusive),
        makeRight(1, 2),
        op
    )).toEqual([
      new Segment(1, at(dur(1))),
