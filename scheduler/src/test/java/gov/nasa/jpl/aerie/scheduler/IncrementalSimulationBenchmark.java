...
public class IncrementalSimulationBenchmark {
  ...
  private static ArrayList<TestSimulatedActivity> getActivities(){
    ...
    for(var cur = Duration.ZERO; cur.shorterThan(durSim); cur = cur.plus(step) ){
      var act = new TestSimulatedActivity(
          cur,
  ...
  private static void benchmarkIncrementalSimulationDriver(){
    final var acts = getActivities();
    final var fooMissionModel = SimulationUtility.getFooMissionModel();
    final IncrementalSimulationDriver incrementalSimulationDriver = new IncrementalSimulationDriver(fooMissionModel);
    int i = 0;
    var sum = 0.;
    for(var act : acts) {
      final var start = System.nanoTime();
      incrementalSimulationDriver.simulateActivity(act.activity, act.start, act.name);
      final var dur = System.nanoTime() - start;
      sum += dur;
      final var curMean = sum / (++i);
      System.out.println(i + " " + curMean);
  ...
  private record TestSimulatedActivity(Duration start, SerializedActivity activity, String name){}
