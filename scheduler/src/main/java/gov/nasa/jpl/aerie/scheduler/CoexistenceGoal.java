...
public class CoexistenceGoal extends ActivityTemplateGoal {
    ...
    public static class Builder extends ActivityTemplateGoal.Builder<Builder> {
        ...
        public Builder forEach( TimeRangeExpression expr ) { forEach = expr; return getThis(); }
        protected TimeRangeExpression forEach;

       public Builder startsAt(TimeExpression timeExpression) { startExpr = timeExpression; return getThis();}
        protected TimeExpression startExpr;

        public Builder endsAt(TimeExpression timeExpression) { endExpr = timeExpression; return getThis();}
        protected TimeExpression endExpr;


        public Builder startsAt(TimeAnchor anchor) { startExpr = TimeExpression.fromAnchor(anchor); return getThis();}

        public Builder endsAt(TimeAnchor anchor) { endExpr = TimeExpression.fromAnchor(anchor); return getThis();}

        public Builder startsAfter(TimeAnchor anchor){ startExpr = TimeExpression.afterStart(); return getThis();}
        public Builder endsBefore(TimeAnchor anchor){ endExpr = TimeExpression.beforeEnd(); return getThis();}

        /**
         * {@inheritDoc}
         */
        @Override public CoexistenceGoal build() { return fill( new CoexistenceGoal() ); }
        ...
        protected CoexistenceGoal fill(CoexistenceGoal goal ) {
            ...
            if( forEach == null ) { throw new IllegalArgumentException(
    ...
    public java.util.Collection<Conflict> getConflicts(Plan plan ) {
        ...
        for(var window: anchors.getRangeSet()){
            ...
            //second part is useless but makes it more readeable
                ...
                if(rangeDur != null){
                    Range<Time> endTime = new Range<Time>(startTimeRange.getMinimum().plus(rangeDur.getMinimum()), startTimeRange.getMaximum().plus(rangeDur.getMaximum()));
                ...
                Range<Duration> rangeDur = desiredActTemplate.getDurationRange();
                if(rangeDur != null){
                    startTimeRange= new Range<Time>(endTimeRange.getMinimum().minus(rangeDur.getMaximum()), endTimeRange.getMaximum().minus(rangeDur.getMinimum()));
                    actTB.startsIn(startTimeRange);
                }
