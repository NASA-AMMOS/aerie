...
public class PlanVisualizer extends JFrame {
   ...
   * Horizon marker (activity type "HorizonMarker") and windows activities (activity type "Window") are treated
   * specifically
   *
   ...
   */
  private void createWindowDataset(
      List<ActivityInstance> windows,
    ...
    for (var act : windows) {
      ...
      for (var listOfWin : nonOverlappingWindows) {
        ...
        for (var actInList : listOfWin) {
          Range<Time> actToCompare = new Range<Time>(
              actInList.getStartTime(),
              actInList.getStartTime().plus(actInList.getDuration()));
   ...
   */
  private TaskSeries getSerieAndSubtasksFromListActs(
      String nameSerie,
      ...
      Time maxEndTime)
  {
    ...
    for (final var act : activities) {
      ...
      Task actst = new Task(
          name,
          Date.from(act.getStartTime().toInstant()),
          Date.from(act.getStartTime().plus(dur).toInstant()));
  ...
  private IntervalCategoryDataset convertPlanToDataset(Plan plan) {
    ...
    for (final var typeAndActs : plan.getActivitiesByType().entrySet()) {
      ...
      TaskSeries serie = getSerieAndSubtasksFromListActs(
          typeAndActs.getKey(),
          typeAndActs.getValue(),
          minStartTime,
          maxEndTime);
