...
public final class SumParsers {
  ...
  public static <T> Variant<T> variant(
      final String tag, final Class<T> klass, final JsonObjectParser<T> parser) {
    return new Variant<>(tag, klass, parser);
  }

  public static <T> JsonParser<T> sumP(
      final String tagField, final Class<T> klass, final List<Variant<? extends T>> variants) {
    return new JsonObjectParser<T>() {
      ...
      public JsonObject getSchema(final SchemaCache anchors) {
        ...
        for (final var variant : variants) {
          ...
          builder.add(
              Json.createObjectBuilder(schema)
                  .add(
                      "properties",
        ...
        return Json.createObjectBuilder().add("oneOf", builder.build()).build();
      }
      ...
      public JsonParseResult<T> parse(final JsonValue json) {
        ...
        if (!obj.containsKey(tagField))
          return JsonParseResult.failure("missing field `%s`".formatted(tagField));
      ...
      public JsonObject unparse(final T value) {
        for (final var variant : variants) {
          ...
          return Json.createObjectBuilder(unsafeParseSubclass(variant, value))
