...
CREATE OR REPLACE VIEW merlin.derived_events
  ...
  SELECT source_id,
    ...
    FROM ( SELECT rule1_3.source_id,
      ...
      rule1_3.slot_end,
      rule1_3.valid_at,
      row_number() OVER (PARTITION BY rule1_3.key, rule1_3.derivation_group_id ORDER BY rule1_3.valid_at DESC) AS rn
      FROM ( SELECT sub.id AS source_id,
        ...
        FROM merlin.external_event
          ...
          ORDER BY sub.derivation_group_id, external_event.start_time) rule1_3
    ) t
    WHERE t.rn = 1
    ORDER BY event_id;
