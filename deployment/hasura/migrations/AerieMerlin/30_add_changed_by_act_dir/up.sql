-- add created_by to act dir
alter table activity_directive
  add column created_by text,
  add constraint activity_directive_created_by_exists
...
-- add created_by to activity_directive_extended view
...
-- add changed_by to anchor deletion functions
...
-- add created_by to plan_snapshot
alter table plan_snapshot_activities
  add column created_by text;

create or replace function create_snapshot(_plan_id integer, _snapshot_name text, _user text)
...
create or replace procedure restore_from_snapshot(_plan_id integer, _snapshot_id integer)
	language plpgsql as $$
	declare
		_snapshot_name text;
		_plan_name text;
