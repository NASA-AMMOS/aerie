...
public final class TypescriptCodeGenerationService {
  ...
  private static final ValueSchema LINEAR_RESOURCE_SCHEMA =
  ...
  public static String generateTypescriptTypes(
      final Map<String, ActivityType> activityTypes, final Map<String, ValueSchema> resources) {
    final var result = new ArrayList<String>();
    result.add("/** Start Codegen */");
    result.add("import * as AST from './constraints-ast.js';");
    result.add(
        "import { Discrete, Real, Windows, ActivityInstance } from"
            + " './constraints-edsl-fluent-api.js';");
    ...
    for (var resource : resources.keySet()) {
      result.add(
          indent(
              "\"" + resource + "\": " + valueSchemaToTypescript(resources.get(resource)) + ","));
    }
    result.add("};");
    ...
    result.add(
        "export type ResourceName = "
            + String.join(
                " | ",
                Stream.concat(
                        resources.keySet().stream().map(key -> ("\"" + key + "\"")),
                        Stream.of("never"))
                    .toList())
            + ";");

    result.add(
        "export type RealResourceName = "
            + String.join(
                " | ",
                Stream.concat(
                        resources.entrySet().stream()
                            .filter($ -> valueSchemaIsNumeric($.getValue()))
                            .map($ -> ("\"" + $.getKey() + "\"")),
                        Stream.of("never"))
                    .toList())
            + ";");
    ...
    for (final var activityType : mapInterfaces.entrySet()) {
      result.add(
          indent("[ActivityType." + activityType.getKey() + "]:" + activityType.getValue() + ","));
    }
    result.add("};");
    ...
    for (final var activityType : activityTypes.entrySet()) {
      ...
      final var interfaceType =
          activityTypeToInterfaceWithUndefined(nameInterface, activityType.getValue());
  ...
  public static String activityTypeToInterface(
      String typeName, TypescriptCodeGenerationService.ActivityType activityType) {
    final var result = new ArrayList<String>();
  ...
  public static String activityTypeToInterfaceWithUndefined(
      String typeName, TypescriptCodeGenerationService.ActivityType activityType) {
    final var result = new ArrayList<String>();
    ...
    return valueSchema.match(
        new ValueSchema.Visitor<>() {
          ...
          public String onInt() {
            ...
            // REVIEW: even on int, we are expecting a Real and not a Discrete<number> so we have
        ...
        new ValueSchema.Visitor<>() {
          ...
          public String onVariant(final List<ValueSchema.Variant> variants) {
            ...
            for (final var variant : variants) {
              result.append(" | \"").append(variant.label()).append("\"");
            }
