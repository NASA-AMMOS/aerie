...
public record ValueAt<P extends Profile<P>>(
    ProfileExpression<P> profile, Expression<Spans> timepoint)
  ...
  public DiscreteProfile evaluate(
    ...
    return new DiscreteProfile(
        IntervalMap.<SerializedValue>builder().set(Segment.of(bounds, value.get())).build());
  }

  @Override
  public String prettyPrint(final String prefix) {
    return String.format(
        "\n%s(valueAt %s %s)", prefix, this.profile.prettyPrint(), timepoint.prettyPrint());
  }
