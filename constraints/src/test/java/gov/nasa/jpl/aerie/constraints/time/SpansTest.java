...
public class SpansTest {
  ...
  public void filter() {
    final var spans =
        new Spans(interval(0, 1, SECONDS), interval(0, 2, SECONDS), interval(0, 3, SECONDS))
            .filter(
                $ ->
                    $.duration().shorterThan(Duration.of(2, SECONDS))
                        || $.duration().longerThan(Duration.of(2, SECONDS)));
  ...
  public void map() {
    final var spans =
        new Spans(interval(0, 2, SECONDS), interval(0, 3, SECONDS))
  ...
  public void mapFiltersEmpty() {
    final var spans =
        new Spans(interval(0, Inclusive, 1, Exclusive, SECONDS), interval(0, 3, SECONDS))
            .map($ -> interval($.start, $.startInclusivity, $.end.minus(SECOND), $.endInclusivity));

    final var expected = List.of(Segment.of(interval(0, 2, SECOND), Optional.empty()));
  ...
  public void flatMap() {
    final var spans =
        new Spans(interval(0, 1, SECONDS), interval(0, 3, SECONDS))
            .flatMap(
                $ -> {
                  if ($.duration().noLongerThan(SECOND)) {
                    return Stream.of();
                  } else {
                    return Stream.of(interval($.end, $.end), interval($.end, $.end.plus(SECOND)));
                  }
                });
        ...
        new Spans(interval(0, Inclusive, 1, Exclusive, SECONDS), interval(0, 3, SECONDS))
            .flatMap(
                $ ->
                    Stream.of(
                        interval(
                            $.start, $.startInclusivity, $.end.minus(SECOND), $.endInclusivity)));
  ...
  public void intoWindows() {
    final var intervals =
        new Spans(interval(0, 2, SECONDS), interval(1, 3, SECONDS), interval(5, 5, SECONDS))
            .intoWindows();
    ...
    final var expected =
        List.of(
            ...
            Segment.of(
                interval(Duration.of(5, SECONDS), Exclusive, Duration.MAX_VALUE, Inclusive),
                false));
    ...
    final var expected =
        new LinearProfile(
            ...
            Segment.of(
                ...
                new LinearEquation(Duration.ZERO, 0, 1)),
            Segment.of(interval(1, 2, SECONDS), new LinearEquation(Duration.SECOND, 1, 2)),
            Segment.of(
                interval(2, Exclusive, 3, Inclusive, SECONDS),
                new LinearEquation(Duration.of(2, SECOND), 3, 1)),
            Segment.of(
                interval(3, Exclusive, 5, Inclusive, SECONDS),
                new LinearEquation(Duration.ZERO, 4, 0)),
            Segment.of(
                interval(5, Exclusive, 6, Inclusive, SECONDS),
                new LinearEquation(Duration.of(5, SECOND), 4, 1)),
            Segment.of(
                interval(Duration.of(6, SECOND), Exclusive, Duration.MAX_VALUE, Inclusive),
                new LinearEquation(Duration.ZERO, 5, 0)));
  ...
  public void testIntersectWindows() {
    final var intersection =
        new Spans(interval(0, 2, SECONDS))
            .intersectWith(new Windows(false).set(interval(1, 10, SECONDS), true));
