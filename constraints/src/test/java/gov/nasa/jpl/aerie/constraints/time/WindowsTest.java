...
public class WindowsTest {
  ...
  public void setAndSetAll() {
    ...
    Windows w = new Windows().set(interval(Duration.ZERO, Duration.MAX_VALUE), false);
  ...
  public void isEmptyVsIsFalse() {
    ...
    // isEmpty is different from isFalse. isEmpty checks is there anything at all - all false would
    // still return false
  ...
  public void unsetMain() {
    ...
    Windows result =
        new Windows(interval(Duration.ZERO, Duration.MAX_VALUE), true)
    ...
    Windows expected =
        new Windows(
            Segment.of(interval(0, 5, SECONDS), true),
            ...
            Segment.of(
                interval(Duration.of(30, SECONDS), Exclusive, Duration.MAX_VALUE, Inclusive),
                true));
  ...
  public void and() {
    ...
    Windows orig =
        new Windows(
            Segment.of(interval(1, 4, SECONDS), true), Segment.of(interval(6, 11, SECONDS), false));
  ...
  public void minMaxTimePoints() {
    ...
    // multiple intervals
    w = w.set(interval(3, 50, SECONDS), true).set(interval(75, 200, SECONDS), true);
    ...
    w = w.unset(at(Duration.MAX_VALUE)).set(at(Duration.ZERO), true);
  ...
  public void filterByDurationNormal1() {
    ...
    Windows orig =
        new Windows(
            Segment.of(interval(0, 4, SECONDS), true),
    ...
    Windows expected =
        new Windows(
            Segment.of(interval(0, 4, SECONDS), false),
            Segment.of(interval(6, 9, SECONDS), false),
            Segment.of(interval(11, 14, SECONDS), true),
  ...
  public void shiftByStretch() {
    ...
    Windows expected =
        new Windows(
            ...
            Segment.of(
                interval(Duration.MAX_VALUE.minus(Duration.of(1, SECONDS)), Duration.MAX_VALUE),
                true));
    assertIterableEquals(expected, result);
  }
  ...
  public void accumulatedDuration() {
    ...
    final var expected =
        new LinearProfile(
            Segment.of(
                interval(Duration.MIN_VALUE, Inclusive, Duration.ZERO, Exclusive),
                new LinearEquation(Duration.ZERO, 0, 0)),
            Segment.of(
                interval(0, Inclusive, 1, Inclusive, SECOND),
                new LinearEquation(Duration.ZERO, 0, 1000)),
            Segment.of(
                interval(1, Exclusive, 2, Exclusive, SECOND), new LinearEquation(SECOND, 1000, 0)),
            Segment.of(
                interval(2, Inclusive, 3, Exclusive, SECOND),
                new LinearEquation(Duration.of(2, SECOND), 1000, 1000)),
            Segment.of(interval(3, 4, SECOND), new LinearEquation(Duration.ZERO, 2000, 0)),
            Segment.of(
                interval(4, Exclusive, 5, Inclusive, SECOND),
                new LinearEquation(Duration.of(4, SECOND), 2000, 1000)),
            Segment.of(
                interval(5, Exclusive, 8, Exclusive, SECOND),
                new LinearEquation(Duration.ZERO, 3000, 0)),
            Segment.of(
                interval(Duration.of(9, SECOND), Exclusive, Duration.MAX_VALUE, Inclusive),
                new LinearEquation(Duration.ZERO, 3000, 0)));
  ...
  public void shiftByConnectedIntervals() {
    ...
    assertEquals(new Windows(interval(-1, Exclusive, 8, Exclusive, SECONDS), false), removal);
  }

  @Test
  public void shiftByDisconnectedPoints() {
    var orig = new Windows().set(at(0, SECONDS), true).set(at(2, SECONDS), false);

    var fromStartPosFromEndPos = orig.shiftBy(Duration.of(-1, SECONDS), Duration.of(1, SECONDS));
    assertIterableEquals(new Windows(interval(-1, 1, SECONDS), true), fromStartPosFromEndPos);

    var fromStartPosFromEndNeg = orig.shiftBy(Duration.of(1, SECONDS), Duration.of(-1, SECONDS));
    assertIterableEquals(new Windows(interval(1, 3, SECONDS), false), fromStartPosFromEndNeg);

    var fromStartNegFromEndPos = orig.shiftBy(Duration.of(1, SECONDS), Duration.of(1, SECONDS));
    assertIterableEquals(
        new Windows().set(at(1, SECONDS), true).set(at(3, SECONDS), false), fromStartNegFromEndPos);

    var fromStartNegFromEndNeg = orig.shiftBy(Duration.of(-1, SECONDS), Duration.of(-1, SECONDS));
    assertIterableEquals(
        new Windows().set(at(-1, SECONDS), true).set(at(1, SECONDS), false),
        fromStartNegFromEndNeg);
  }
  ...
  public void iterator() {
    var nw = new Windows().set(interval(1, 2, SECONDS), true);
  ...
  public void intoSpans() {
    ...
    var expected = new Spans(interval(0, 3, SECONDS), interval(5, 5, SECONDS));

    assertIterableEquals(expected, windowsWithoutGaps.intoSpans(Interval.FOREVER));

    expected = new Spans(interval(1, 2, SECONDS));
    assertIterableEquals(expected, windowsWithGaps.intoSpans(interval(1, 2, SECONDS)));
  }
}
