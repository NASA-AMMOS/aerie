...
public class WindowsTest {
  ...
  public void constructorTests() {
    ...
    var windows3 = new Windows(
        Pair.of(interval(Duration.MIN_VALUE, Duration.of(0, SECONDS)), true),
        Pair.of(at(1, SECONDS), true),
        Pair.of(interval(2, 3, SECONDS), false),
        Pair.of(at(3, SECONDS), true)
    );
  ...
  public void setAndSetAll() {
    ...
    w.set(interval(Duration.ZERO, Duration.MAX_VALUE), false);

    //added correctly?
    assertEquals(w.size(), 1);

    //add at front, back, in between, make sure they're added correctly
    w.set(at(Duration.ZERO), true); //don't coalesce
    w.set(at(Duration.MAX_VALUE), false); //coalesce implictly, nothing happens w/ this line
    w.set(interval(3, 50, SECONDS), true);
  ...
  public void isEmptyVsIsFalse() {
    ...
    Windows soTrue = new Windows(Pair.of(interval(3, 5, SECONDS), true),
                                 Pair.of(interval(7, 10, SECONDS), true),
                                 Pair.of(interval(12, 15, SECONDS), true),
                                 Pair.of(interval(30, 35, SECONDS), true));
    ...
    assertTrue(StreamSupport.stream(soTrue.spliterator(), false).allMatch(Pair::getValue));
  ...
  public void unsetMain() {
    ...
    result.unset(interval(5, Exclusive, 7, Inclusive, SECONDS));
    result.unsetAll(List.of(interval(9, Exclusive, 12, Inclusive, SECONDS)));
    result.unsetAll(List.of(interval(13, Exclusive, 15, Inclusive, SECONDS)));
    result.unsetAll(
        List.of(
    ...
    result.unset(interval(29, 30, SECONDS));
    result.unset(interval(29, 30, SECONDS));

    //values of result should never have changed!
    assertTrue(StreamSupport.stream(result.spliterator(), false).allMatch(Pair::getValue));

    //check values
    Windows expected = new Windows(Pair.of(interval(0, 5, SECONDS), true),
                                   Pair.of(interval(7, Exclusive, 9, Inclusive, SECONDS), true),
                                   Pair.of(interval(12, Exclusive, 13, Inclusive, SECONDS), true),
                                   Pair.of(interval(15, Exclusive, 20, Inclusive, SECONDS), true),
                                   Pair.of(interval(25, Inclusive, 26, Inclusive, SECONDS), true),
                                   Pair.of(interval(27, Inclusive, 29, Exclusive, SECONDS), true),
                                   Pair.of(interval(Duration.of(30, SECONDS), Exclusive,
  ...
  public void and() {
    ...
    Windows orig = new Windows(
        Pair.of(interval(1, 4, SECONDS), true),
        Pair.of(interval(6, 11, SECONDS), false)
    );

    Windows intersectMe = new Windows(
        Pair.of(interval(0, 2, SECONDS), true),
        Pair.of(interval(2, Exclusive, 3, Inclusive, SECONDS), false),
        Pair.of(interval(7, 8, SECONDS), true),
        Pair.of(interval(9, 10, SECONDS), false),
        Pair.of(interval(12, 13, SECONDS), true),
        Pair.of(interval(14, 15, SECONDS), false)
    );
    Windows intersection = orig.and(intersectMe);

    Windows expected = new Windows(
        Pair.of(interval(1, 2, SECONDS), true),
        Pair.of(interval(2, Exclusive, 3, Inclusive, SECONDS), false),
        Pair.of(interval(6, 11, SECONDS), false),
        Pair.of(interval(14, 15, SECONDS), false)
    );
  ...
  public void minMaxTimePoints() {
    ...
    //multiple intervals
    w.set(interval(3, 50, SECONDS), true);
    w.set(interval(75, 200, SECONDS), true);
    assertEquals(w.minValidTimePoint().get(), Pair.of(Duration.ZERO, Inclusive));
    assertEquals(w.maxValidTimePoint().get(), Pair.of(Duration.MAX_VALUE, Inclusive));
    ...
    w.unset(at(Duration.MAX_VALUE));
    w.set(at(Duration.ZERO), true);
    assertEquals(w.minValidTimePoint().get(), Pair.of(Duration.ZERO, Inclusive));
  ...
  public void complement() {
    Windows main = new Windows(
        Pair.of(at(0, SECONDS), false),
        Pair.of(interval(1, 3, SECONDS), true),
        Pair.of(interval(4, 7, SECONDS), false),
        Pair.of(interval(8, 10, SECONDS), true),
        Pair.of(at(12, SECONDS), true),
        Pair.of(at(13, SECONDS), false),
        Pair.of(at(Duration.MAX_VALUE), true)
    );
    main.unset(at(9, SECONDS));

    main = main.not();

    //check values
    Windows expected = new Windows(
        Pair.of(at(0, SECONDS), true),
        Pair.of(interval(1, 3, SECONDS), false),
        Pair.of(interval(4, 7, SECONDS), true),
        Pair.of(interval(8, Inclusive, 9, Exclusive, SECONDS), false),
        Pair.of(interval(9, Exclusive, 10, Inclusive, SECONDS), false),
        Pair.of(at(12, SECONDS), false),
        Pair.of(at(13, SECONDS), true),
        Pair.of(at(Duration.MAX_VALUE), false)
    );
  ...
  public void filterByDurationNormal1() {
    ...
    Windows orig = new Windows(Pair.of(interval(0, 4, SECONDS), true),
                               Pair.of(interval(6, 9, SECONDS), false),
                               Pair.of(interval(11, 14, SECONDS), true),
                               Pair.of(interval(16, 17, SECONDS), true));


    orig = orig.filterByDuration(Duration.of(3, SECONDS), Duration.of(3, SECONDS));

    Windows expected = new Windows(Pair.of(interval(0, 4, SECONDS), false),
                                   Pair.of(interval(6, 9, SECONDS), false),
                                   Pair.of(interval(11, 14, SECONDS), true),
                                   Pair.of(interval(16, 17, SECONDS), false));
  ...
  public void filterByDurationZero() {
    ...
    Windows expected = new Windows(Pair.of(interval(0, 4, SECONDS), false),
                                   Pair.of(interval(6, 9, SECONDS), false),
                                   Pair.of(interval(11, 14, SECONDS), false),
                                   Pair.of(interval(16, 17, SECONDS), false));
    assertIterableEquals(expected, orig);
  }
  ...
  public void filterByDurationMinZeroMaxMax() {
    ...
    Windows expected = new Windows(Pair.of(interval(0, 4, SECONDS), true),
                                   Pair.of(interval(6, 9, SECONDS), false),
                                   Pair.of(interval(11, 14, SECONDS), true),
                                   Pair.of(interval(16, 17, SECONDS), true));
    assertIterableEquals(expected, orig);
  ...
  public void filterByDurationMinMaxMaxMax() {
    ...
    final var orig = new Windows(Pair.of(interval(0, 4, SECONDS), true),
                               Pair.of(interval(6, 9, SECONDS), false),
                               Pair.of(interval(11, 14, SECONDS), true),
                               Pair.of(interval(16, 17, SECONDS), true));
  ...
  public void filterByDurationMinMaxMaxZero() {
    ...
    final Windows orig = new Windows(Pair.of(interval(0, 4, SECONDS), true),
                               Pair.of(interval(6, 9, SECONDS), false),
                               Pair.of(interval(11, 14, SECONDS), true),
                               Pair.of(interval(16, 17, SECONDS), true));
  ...
  public void shiftByStretch() {
    Windows orig = new Windows(Pair.of(at(0, SECONDS), true),
                               Pair.of(interval(1, 2, SECONDS), false),
                               Pair.of(interval(5, 6, SECONDS), true),
                               Pair.of(interval(7, 8, SECONDS), false),
                               Pair.of(interval(8, Exclusive, 10, Exclusive, SECONDS), true),
                               Pair.of(interval(13, 14, SECONDS), true),
                               Pair.of(interval(14, Exclusive, 16, Exclusive, SECONDS), false),
                               Pair.of(at(Duration.MAX_VALUE.minus(Duration.of(1, SECONDS))), true)); //long overflow if at max value

    Windows result = orig.shiftBy(Duration.of(-1, SECONDS), Duration.of(1, SECONDS));


    Windows expected = new Windows(Pair.of(interval(-1, 1, SECONDS), true),
                                   Pair.of(interval(1, Exclusive, 2, Inclusive, SECONDS), false),
                                   Pair.of(interval(4, Inclusive, 11, Exclusive, SECONDS), true),
                                   Pair.of(interval(12, 15, SECONDS), true),
                                   Pair.of(interval(15, Exclusive, 16, Exclusive, SECONDS), false),
                                   Pair.of(interval(Duration.MAX_VALUE.minus(Duration.of(2, SECONDS)),
  ...
  public void shiftByFromStartFromEndPermsWithInterval() {
    ...
    orig.setAll(new Windows(Pair.of(leftEnd, true), Pair.of(rightEnd, true)));

    var fromStartPosFromEndPos = orig.shiftBy(Duration.of(-1, SECONDS), Duration.of(1, SECONDS));
    assertEquals(fromStartPosFromEndPos, new Windows(
    ...
    assertEquals(fromStartPosFromEndNeg, new Windows(
        ...
        Pair.of(interval(1, 1, SECONDS), true),
        Pair.of(interval(1, Exclusive, 2, Inclusive, SECONDS), false),
        Pair.of(interval(8, Inclusive, 9, Exclusive, SECONDS), false),
        Pair.of(interval(9, 9, SECONDS), true),
        Pair.of(interval(9, Exclusive, 10, Inclusive, SECONDS), false)
    ));
    ...
    assertEquals(fromStartNegFromEndPos, new Windows(
        ...
        Pair.of(interval(1, 3, SECONDS), true),
        Pair.of(interval(8, Inclusive, 9, Exclusive, SECONDS), false),
        Pair.of(interval(9, 10, SECONDS), true)
    ));
    ...
    assertEquals(fromStartNegFromEndNeg, new Windows(
        Pair.of(interval(0, 1, SECONDS), true),
        Pair.of(interval(1, Exclusive, 2, Inclusive, SECONDS), false),
        Pair.of(interval(7, 9, SECONDS), true),
        Pair.of(interval(9, Exclusive, 10, Inclusive, SECONDS), false)
    ));

    var removal = orig.shiftBy(Duration.of(0, SECONDS), Duration.of(-3, SECONDS));
    assertEquals(new Windows(
        Pair.of(interval(0, 2, SECONDS), false),
        Pair.of(interval(8, 10, SECONDS), false)
    ), removal);
  }
  ...
  public void shiftByFromStartFromEndPermsWithPoint() {
    ...
    var fromStartPosFromEndPos = orig.shiftBy(Duration.of(-1, SECONDS), Duration.of(1, SECONDS));
    assertEquals(fromStartPosFromEndPos, new Windows(Pair.of(interval(0, 1, SECONDS), true),
                                                     Pair.of(interval(3, 4, SECONDS), true)));

    var fromStartPosFromEndNeg = orig.shiftBy(Duration.of(1, SECONDS), Duration.of(-1, SECONDS));
    assertEquals(fromStartPosFromEndNeg, new Windows(Pair.of(leftEnd, false), Pair.of(rightEnd, false)));

    var fromStartNegFromEndPos = orig.shiftBy(Duration.of(1, SECONDS), Duration.of(1, SECONDS));
    assertEquals(fromStartNegFromEndPos, new Windows(Pair.of(leftEnd, false), Pair.of(interval(1, 1, SECONDS), true), Pair.of(rightEnd, false)));

    var fromStartNegFromEndNeg = orig.shiftBy(Duration.of(-1, SECONDS), Duration.of(-1, SECONDS));
    assertEquals(fromStartNegFromEndNeg, new Windows(Pair.of(leftEnd, false), Pair.of(interval(3, 3, SECONDS), true), Pair.of(rightEnd, false)));
  ...
  public void includes() {
    ...
    var w = new Windows(
        Pair.of(interval(0, 2, SECONDS), true),
        Pair.of(interval(4, 5, SECONDS), false),
        Pair.of(interval(5, 6, SECONDS), true)
    );
    ...
    assertTrue(w.includes(new Windows(
        Pair.of(interval(0, 1, SECONDS), true),
        Pair.of(interval(5, 6, SECONDS), true),
        Pair.of(interval(3, 5, SECONDS), false)
    )));
  ...
  public void iterator() {
    ...
    nw.set(interval(1, 2, SECONDS), true);

    //some simple iterator tests
    var iter = nw.iterator();
    iter.forEachRemaining($ -> assertEquals($, Pair.of(interval(1, 2, SECONDS), true)));

    iter = nw.iterator(); //everything has been consumed - need to reset
    assertEquals(iter.next(), Pair.of(interval(1, 2, SECONDS), true));
  ...
  public void intoSpans() {
    final var spans = new Windows(
        Pair.of(interval(0, 2, SECONDS), true),
        Pair.of(interval(1, 3, SECONDS), true),
        Pair.of(interval(5, 5, SECONDS), true),
        Pair.of(interval(6, 8, SECONDS), false)
    ).intoSpans();
