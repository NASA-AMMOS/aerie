...
public final class RecursiveEventGraphEvaluator implements EventGraphEvaluator {
  ...
  private static <Effect> Optional<Effect> sequence(
      final EffectTrait<Effect> trait, final Optional<Effect> a, final Optional<Effect> b) {
  ...
  private static <Effect> Optional<Effect> merge(
      final EffectTrait<Effect> trait, final Optional<Effect> a, final Optional<Effect> b) {
    if (a.isEmpty()) return b;
    if (b.isEmpty()) return a;
