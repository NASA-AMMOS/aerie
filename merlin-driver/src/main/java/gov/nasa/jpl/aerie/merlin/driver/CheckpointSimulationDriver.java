...
public class CheckpointSimulationDriver {
    ...
    try {
      ...
      while (elapsedTime.noLongerThan(simulationDuration) && !simulationCanceled.get()) {
        ...
        if (shouldTakeCheckpoint.apply(new SimulationState(elapsedTime, nextTime, engine, schedule, activityToTask))) {
          if(duplicationIsOk) cells.freeze();
          ...
          final var newCachedEngine = new CachedSimulationEngine(
              ...
              activityTopic,
              missionModel);
          if(duplicationIsOk) newCachedEngine.freeze();
          cachedEngineStore.save(
              newCachedEngine,
              configuration);
          timelines.add(timeline);
          engine = duplicationIsOk ? engine.duplicate() : engine;
  ...
  private static <Model> Map<ActivityDirectiveId, TaskId> scheduleActivities(
      ...
      final Map<ActivityDirectiveId, TaskId> activityToTask,
      final Topic<ActivityDirectiveId> activityTopic){
    final var toCheckForDependencyScheduling = new HashMap<ActivityDirectiveId, TaskId>();
