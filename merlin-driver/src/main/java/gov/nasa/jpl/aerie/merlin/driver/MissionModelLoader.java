...
public final class MissionModelLoader {
  public static ModelType<?, ?> loadModelType(
      final Path path, final String name, final String version) throws MissionModelLoadException {
  ...
  private static <Config, Model> MissionModel<Model> loadMissionModel(
      final Instant planStart,
      final SerializedValue missionModelConfig,
    ...
    try {
      final var serializedConfigMap =
          missionModelConfig
              .asMap()
              .orElseThrow(() -> new InstantiationException.Builder("Configuration").build());
  ...
  public static MerlinPlugin loadMissionModelProvider(
      final Path path, final String name, final String version) throws MissionModelLoadException {
    // Look for a MerlinPlugin implementor in the mission model. For correctness, we're assuming
    // there's
  ...
  private static String getImplementingClassName(
    ...
    try (final var jarFile = new JarFile(jarPath.toFile())) {
      final var jarEntry =
          jarFile.getEntry("META-INF/services/" + MerlinPlugin.class.getCanonicalName());
  ...
  private static URL missionModelPathToUrl(final Path path) {
    try {
      ...
      // This exception only happens if there is no URL protocol handler available to represent a
      // Path.
    ...
    private MissionModelLoadException(
      ...
      super(
          String.format(
              ...
              MerlinPlugin.class.getSimpleName(), path, name, version),
          cause);
    }
  }
