...
public final class ValueSchemaJsonParser implements JsonParser<ValueSchema> {
  ...
  public JsonParseResult<ValueSchema> parse(final JsonValue json) {
    if (!json.getValueType().equals(JsonValue.ValueType.OBJECT))
      return JsonParseResult.failure("Expected object");
    ...
    if (!type.getValueType().equals(JsonValue.ValueType.STRING))
      return JsonParseResult.failure("\"type\" field must be a string");
  ...
  private JsonParseResult<ValueSchema> parseSeries(final JsonObject obj) {
    if (!obj.containsKey("items"))
      return JsonParseResult.failure("\"series\" value schema requires field \"items\"");
  ...
  private JsonParseResult<ValueSchema> parseStruct(final JsonObject obj) {
    if (!obj.containsKey("items"))
      return JsonParseResult.failure("\"struct\" value schema requires field \"items\"");
    final var items = obj.get("items");
    if (!items.getValueType().equals(JsonValue.ValueType.OBJECT))
      return JsonParseResult.failure("\"items\" field of \"struct\" must be an object");
  ...
  private JsonParseResult<ValueSchema> parseVariant(final JsonObject obj) {
    final JsonParser<ValueSchema.Variant> variantP =
        productP
            .field("key", stringP)
            .field("label", stringP)
            .map(untuple(ValueSchema.Variant::new), $ -> tuple($.key(), $.label()));
