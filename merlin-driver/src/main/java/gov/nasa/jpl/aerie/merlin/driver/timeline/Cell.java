...
public final class Cell<State> {
  ...
  public <Effect> Cell(
    ...
    this(new GenericCell<>(applicator, algebra, selector), state);
  }
  ...
  private record GenericCell<Effect, State> (
    ...
    public void apply(final State state, final EventGraph<Event> events) {
      this.applicator.apply(state, events.evaluate(this.algebra, this::select));
    }

    private Effect select(final Event event$) {
      return this.selector.select(event$, this.algebra::empty);
    }
  }
}
