...
import { HbCommand, HbCommandParameter, StringTMap } from '../models';
 ...
 */
export function keyCommandsByName(
  commands: HbCommand[],
): StringTMap<HbCommand> {
  const newCommands: StringTMap<HbCommand> = {};
  ...
  if (commandsByName.hasOwnProperty(name)) {
    if (parameters.length < commandsByName[name].parameters.length) {
      ...
      commandsByName[name].parameters.forEach(
        (param: HbCommandParameter, i: number) => {
          ...
          switch (param.type) {
            case 'string':
            case 'boolean':
              if (param.regex && !param.regex.exec(inputParam)) {
                res.push({
                  level: 'error',
                  lineNumber,
