...
const { combineLatest, Observable } = require('rxjs');

/**
 * Make a `git` call to get the current branch.
 */
const branch$ = new Observable((observer: any) => {
  exec(
 ...
 */
const commit$ = new Observable((observer: any) => {
  exec(
 ...
 */
combineLatest(branch$, commit$).subscribe((res: string[]) => {
  const [branch, commit] = res;
