...
commandExpansionRouter.post('/put-expansion', async (req, res, next) => {
  ...
  const { rows } = await db.query(
    `
    insert into expansion_rule (activity_type, expansion_logic, parcel_id,
  ...
  const { rows } = await db.query(
    `
        with expansion_set_id as (
          insert into expansion_set (parcel_id, mission_model_id, description, owner, name)
            values ($1, $2, $3, $4, $5)
            returning id),
...
commandExpansionRouter.post('/expand-all-activity-instances', async (req, res, next) => {
  ...
  if (seqToSimulatedActivity.rows.length > 0) {
    ...
    for (const seqRow of seqRows.rows) {
      ...
      const { rows } = await db.query(
        `
          insert into expanded_sequences (expansion_run_id, seq_id, simulation_dataset_id, expanded_sequence)
            values ($1, $2, $3, $4)
            returning id
      `,
