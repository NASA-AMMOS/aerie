...
describe('upload command dictionary', () => {
  it('should upload a command dictionary and all of the fields should be populated correctly', async () => {
    ...
    const { id, path, mission, parsed_json } = await insertCommandDictionary(graphqlClient);

    expect(path).toBe(`/usr/src/app/sequencing_file_store/${mission}/command_lib.${mission}.ts`);
