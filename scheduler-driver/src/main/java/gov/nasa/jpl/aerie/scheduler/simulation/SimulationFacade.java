...
public class SimulationFacade implements AutoCloseable {
  ...
  public gov.nasa.jpl.aerie.constraints.model.SimulationResults
  ...
  public SimulationFacade(
      final PlanningHorizon planningHorizon, final MissionModel<?> missionModel) {
    this.missionModel = missionModel;
    this.planningHorizon = planningHorizon;
    this.driver =
        new ResumableSimulationDriver<>(missionModel, planningHorizon.getAerieHorizonDuration());
  ...
  public Optional<Duration> getActivityDuration(
      final SchedulingActivityDirective schedulingActivityDirective) {
    if (!planActDirectiveIdToSimulationActivityDirectiveId.containsKey(
    ...
    final var duration =
        driver.getActivityDuration(
            planActDirectiveIdToSimulationActivityDirectiveId.get(
                schedulingActivityDirective.getId()));
    if (duration.isEmpty()) {
      logger.error(
          "Simulation is probably outdated, check that no activity is removed between simulation"
              + " and querying");
    }
    return duration;
  }

  private ActivityDirectiveId getIdOfRootParent(
      SimulationResults results, SimulatedActivityId instanceId) {
  ...
  public Map<SchedulingActivityDirective, SchedulingActivityDirectiveId> getAllChildActivities(
      final Duration endTime) {
    computeSimulationResultsUntil(endTime);
    final Map<SchedulingActivityDirective, SchedulingActivityDirectiveId> childActivities =
        new HashMap<>();
    this.lastSimDriverResults.simulatedActivities.forEach(
        (activityInstanceId, activity) -> {
          ...
          final var schedulingActId =
              planActDirectiveIdToSimulationActivityDirectiveId.entrySet().stream()
                  .filter(entry -> entry.getValue().equals(rootParent))
                  .findFirst()
                  .get()
                  .getKey();
  ...
  public void removeActivitiesFromSimulation(
      final Collection<SchedulingActivityDirective> activities) throws SimulationException {
   ...
   */
  public void replaceActivityFromSimulation(
    ...
    if (toBeReplaced.type() != replacement.type()
      ...
      throw new IllegalArgumentException(
          "When replacing an activity, you can only update the duration");
    }
    if (!insertedActivities.containsKey(toBeReplaced)) {
      throw new IllegalArgumentException(
          "Trying to replace an activity that has not been previously simulated");
    }
    ...
    final var simulationId =
        this.planActDirectiveIdToSimulationActivityDirectiveId.get(toBeReplaced.id());
  ...
  public void simulateActivities(final Collection<SchedulingActivityDirective> activities) {
    final var activitiesSortedByStartTime =
        activities.stream()
            .sorted(Comparator.comparing(SchedulingActivityDirective::startOffset))
            .toList();
  ...
  public void simulateActivity(final SchedulingActivityDirective activity)
  ...
  public void computeSimulationResultsUntil(final Duration endTime) {
    ...
    final var results =
        driver.getSimulationResultsUpTo(this.planningHorizon.getStartInstant(), endTimeWithMargin);
    // compare references
    if (results != lastSimDriverResults) {
      // simulation results from the last simulation, as converted for use by the constraint
      // evaluation engine
      lastSimConstraintResults =
          SimulationResultsConverter.convertToConstraintModelResults(
              results, planningHorizon.getAerieHorizonDuration());
  ...
  private ActivityDirective schedulingActToActivityDir(SchedulingActivityDirective activity) {
    if (activity.getParentActivity().isPresent()) {
      throw new Error(
          "This method should not be called with a generated activity but with its top-level"
              + " parent.");
    }
    ...
    if (activity.duration() != null) {
      ...
      if (durationType instanceof DurationType.Controllable dt) {
        arguments.put(
            dt.parameterName(), SerializedValue.of(activity.duration().in(Duration.MICROSECONDS)));
      } else if (durationType instanceof DurationType.Uncontrollable
          || durationType instanceof DurationType.Fixed) {
        // If an activity has already been simulated, it will have a duration, even if its
        // DurationType is Uncontrollable.
      } else {
