  ...
  public SerializedValue getValue(SimulationResults simulationResults, Plan plan, Interval win) {
    ...
    if (simulationResults.realProfiles.containsKey(this.resourceName)) {
      ...
      final var matchPiece =
          profile.profilePieces.stream().filter($ -> $.interval().contains(queryT)).findFirst();
      return matchPiece
          .map(Segment::value)
          .orElseThrow(
              () ->
                  new Error(
                      "The resource " + this.resourceName + " has no value at time " + queryT));
    } else {
