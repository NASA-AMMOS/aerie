 ...
 */
public class OldSimulationFacade implements AutoCloseable, SimulationFacade {
  ...
  throws SimulationException, SchedulingInterruptedException {
    ...
    try {
      ...
      if(lastSimulationData == null || results != lastSimulationData.driverResults()) {
        ...
        this.lastSimulationData = new SimulationData(plan, results, SimulationResultsConverter.convertToConstraintModelResults(results), planActDirectiveIdToSimulationActivityDirectiveId);
      }
    } catch (SchedulingInterruptedException e){
      throw e; //pass interruption up
    } catch (Exception e){
  ...
  private AugmentedSimulationResultsComputerInputs simulateNoResults(
      ...
      final SchedulingActivityDirective activity) throws SimulationFacade.SimulationException, SchedulingInterruptedException
  {
    ...
    // TODO Determine whether we need to start over

    computeSimulationResultsUntil();
  ...
  @Override
  public SimulationData simulateWithResults(final Plan plan, final Duration until, final Set<String> resourceNames)
  throws SimulationFacade.SimulationException, SchedulingInterruptedException
  {
    if(this.initialSimulationResults.isPresent()) {
      ...
      final var initialPlanA = scheduleFromPlan(this.initialSimulationResults.get().plan(), planningHorizon.getEndAerie(), planningHorizon.getStartInstant(), planningHorizon.getStartInstant());
      if (initialPlanA.equals(inputPlan)) {
        return new SimulationData(
            plan,
            initialSimulationResults.get().driverResults(),
            SimulationResultsConverter.convertToConstraintModelResults(initialSimulationResults.get().driverResults()),
            this.initialSimulationResults.get().mapping());
      }
    }
