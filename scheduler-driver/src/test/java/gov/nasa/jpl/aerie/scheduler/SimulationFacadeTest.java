...
public class SimulationFacadeTest {
  ...
  private PlanInMemory makeTestPlanP0B1() {
    ...
    var act2 =
        SchedulingActivityDirective.of(
            actTypeBite, t2, null, Map.of("biteSize", SerializedValue.of(0.1)), null, true);
    plan.add(act2);
  ...
  public void associationToExistingSatisfyingActivity() {
    ...
    final var goal =
        new CoexistenceGoal.Builder()
            ...
            .forAllTimeIn(
                new WindowsWrapperExpression(new Windows(false).set(horizon.getHor(), true)))
            .thereExistsOne(
    ...
    final var actAssociatedInFirstRun =
        plan1.get().getEvaluation().forGoal(goal).getAssociatedActivities();
    ...
    final var actAssociatedInSecondRun =
        plan2.get().getEvaluation().forGoal(goal).getAssociatedActivities();
    assertThat(actAssociatedInSecondRun.size()).isEqualTo(1);
    assertThat(
            actAssociatedInFirstRun
                .iterator()
                .next()
                .equalsInProperties(actAssociatedInSecondRun.iterator().next()))
        .isTrue();
  }
  ...
  public void getValueAtTimeDoubleOnSimplePlanMidpoint()
    ...
    final var stateQuery =
        new StateQueryParam(getFruitRes().name, new TimeExpressionConstant(t1_5));
    final var actual =
        stateQuery.getValue(facade.getLatestConstraintSimulationResults(), null, horizon.getHor());
  ...
  public void whenValueAboveDoubleOnSimplePlan() throws SimulationFacade.SimulationException {
    ...
    facade.computeSimulationResultsUntil(tEnd);
    var actual =
        new GreaterThan(getFruitRes(), new RealValue(2.9))
            .evaluate(facade.getLatestConstraintSimulationResults());
    var expected =
        new Windows(
  ...
  public void whenValueBelowDoubleOnSimplePlan() throws SimulationFacade.SimulationException {
    ...
    facade.computeSimulationResultsUntil(tEnd);
    var actual =
        new LessThan(getFruitRes(), new RealValue(3.0))
            .evaluate(facade.getLatestConstraintSimulationResults());
    var expected =
        new Windows(
  ...
  public void whenValueBetweenDoubleOnSimplePlan() throws SimulationFacade.SimulationException {
    ...
    var actual =
        new And(
                new GreaterThanOrEqual(getFruitRes(), new RealValue(3.0)),
                new LessThanOrEqual(getFruitRes(), new RealValue(3.99)))
            .evaluate(facade.getLatestConstraintSimulationResults());
    var expected =
        new Windows(
  ...
  public void whenValueEqualDoubleOnSimplePlan() throws SimulationFacade.SimulationException {
    ...
    facade.computeSimulationResultsUntil(tEnd);
    var actual =
        new Equal<>(getFruitRes(), new RealValue(3.0))
            .evaluate(facade.getLatestConstraintSimulationResults());
    var expected =
        new Windows(
            Segment.of(interval(0, Inclusive, 1, Exclusive, SECONDS), false),
  ...
  public void whenValueNotEqualDoubleOnSimplePlan() throws SimulationFacade.SimulationException {
    ...
    facade.computeSimulationResultsUntil(tEnd);
    var actual =
        new NotEqual<>(getFruitRes(), new RealValue(3.0))
            .evaluate(facade.getLatestConstraintSimulationResults());
    var expected =
        new Windows(
  ...
  public void testProceduralGoalWithResourceConstraint()
    ...
    SchedulingActivityDirective act1 =
        SchedulingActivityDirective.of(actTypePeel, t0, Duration.ZERO, null, true);

    SchedulingActivityDirective act2 =
        SchedulingActivityDirective.of(actTypePeel, t2, Duration.ZERO, null, true);

    // create an "external tool" that insists on a few fixed activities
    final var externalActs = java.util.List.of(act1, act2);
    final Function<Plan, Collection<SchedulingActivityDirective>> fixedGenerator =
        (p) -> externalActs;

    final var proceduralGoalWithConstraints =
  ...
  public void testActivityTypeWithResourceConstraint() throws SimulationFacade.SimulationException {
    ...
    final var proceduralgoalwithoutconstraints =
        new ProceduralCreationGoal.Builder()
            .forAllTimeIn(
                new WindowsWrapperExpression(new Windows(false).set(horizon.getHor(), true)))
            .generateWith(fixedGenerator)
