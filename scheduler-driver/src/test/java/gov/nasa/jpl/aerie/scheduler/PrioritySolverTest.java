...
public class PrioritySolverTest {
  ...
  private static final PlanningHorizon h =
      new PlanningHorizon(
          TimeUtility.fromDOY("2025-001T01:01:01.001"),
          TimeUtility.fromDOY("2025-005T01:01:01.001"));
  ...
  private static final Duration t3hr = t0.plus(d1hr.times(2));

  private static final NullPointerTester NULL_POINTER_TESTER =
      new NullPointerTester().setDefault(Problem.class, new Problem(null, h, null, null));
  ...
  private static final Correspondence<SchedulingActivityDirective, SchedulingActivityDirective>
      equalExceptInName =
          Correspondence.from(SchedulingActivityDirective::equalsInProperties, "matches");
  ...
  public void getNextSolution_proceduralGoalCreatesActivities() {
    ...
    final var expectedPlan = makePlanA012(problem);
    final var goal =
        new ProceduralCreationGoal.Builder()
            .named("g0")
  ...
  public void getNextSolution_recurrenceGoalWorks() {
    ...
    assertThat(
            plan.getActivitiesByTime()
                .get(0)
                .equalsInProperties(expectedPlan.getActivitiesByTime().get(0)))
        .isTrue();
    assertThat(plan.getActivitiesByTime())
        .comparingElementsUsing(equalExceptInName)
        .containsExactlyElementsIn(expectedPlan.getActivitiesByTime())
  ...
  public void getNextSolution_coexistenceGoalOnActivityWorks() {
    ...
    final var goal =
        new CoexistenceGoal.Builder()
            .named("g0")
            .forAllTimeIn(new WindowsWrapperExpression(new Windows(false).set(h.getHor(), true)))
            .forEach(new ActivityExpression.Builder().ofType(actTypeA).build())
            .thereExistsOne(
                new ActivityCreationTemplate.Builder().ofType(actTypeB).duration(d1min).build())
  ...
  public void testCardGoalWithApplyWhen() {
    ...
    // and the goal window in between [0, 50 min[ so the activities already present in the plan
    // can't count towards satisfying the goal
    final var goalWindow =
        new Windows(false)
            .set(
                List.of(
                    Interval.between(
                        Duration.of(0, Duration.SECONDS), Duration.of(50, Duration.MINUTE))),
                true);

    CardinalityGoal cardGoal =
        new CardinalityGoal.Builder()
            .duration(
                Interval.between(Duration.of(2, Duration.SECONDS), Duration.of(65, Duration.HOUR)))
    ...
    // will insert an activity at the beginning of the plan in addition of the two already-present
    // activities
