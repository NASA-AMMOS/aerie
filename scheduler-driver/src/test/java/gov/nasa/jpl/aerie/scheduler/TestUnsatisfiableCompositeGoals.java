...
public class TestUnsatisfiableCompositeGoals {
  ...
  private static final PlanningHorizon h =
      new PlanningHorizon(
          TimeUtility.fromDOY("2025-001T00:00:00.000"),
          TimeUtility.fromDOY("2025-002T00:00:00.000"));

  private static final Duration t0 = h.getStartAerie();
  private static final Duration d1min = Duration.of(1, Duration.MINUTE);
  private static final Duration d1hr = Duration.of(1, Duration.HOUR);
  private static final Duration t1hr = t0.plus(d1hr);
  private static final Duration t2hr = t0.plus(d1hr.times(2));
  ...
  private static Problem makeTestMissionAB() {
    ...
    return new Problem(
        fooMissionModel,
        h,
        new SimulationFacade(h, fooMissionModel),
  ...
  public CoexistenceGoal BForEachAGoal(ActivityType A, ActivityType B) {
    ...
    return new CoexistenceGoal.Builder()
        ...
        .forEach(new ActivityExpression.Builder().ofType(A).build())
        .thereExistsOne(new ActivityCreationTemplate.Builder().ofType(B).build())
  ...
  public void testAndWithoutBackTrack() {
    ...
    final var compositeAndGoal =
        new CompositeAndGoal.Builder()
            .and(goal2)
            .and(goal)
            .forAllTimeIn(new WindowsWrapperExpression(new Windows(false).set(h.getHor(), true)))
            .build();
    final var exclusionBasic =
        TestUtility.createExclusionSchedulingZone(
            actTypeBasic,
            new Windows(
    ...
    // by default, goals are not backtracking so the plan should show 2 BAR activities and 1 basic
    // activity and the 2 controllable acts from the base plan
  ...
  public void testAndWithBackTrack() {
    ...
    final var plan = solver.getNextSolution().orElseThrow();
    // AND goal is unsatisfiable because of exclusion zone for the Basic activities
    // and goal is backtracking here so the plan should show only the 2 controllable acts from the
    // base plan
    Assertions.assertTrue(TestUtility.activityStartingAtTime(plan, t1hr, actTypeControllable));
    Assertions.assertTrue(TestUtility.activityStartingAtTime(plan, t2hr, actTypeControllable));
  ...
  public void testOrWithoutBacktrack() {
    ...
    // goal is not configured to backtrack plan should show 2 controllable acts from the base plan +
    // the two activities that could be scheduled out of the exclusion zone
    Assertions.assertTrue(TestUtility.activityStartingAtTime(plan, t1hr, actTypeControllable));
    Assertions.assertTrue(TestUtility.activityStartingAtTime(plan, t2hr, actTypeControllable));
    Assertions.assertTrue(TestUtility.activityStartingAtTime(plan, t2hr, actTypeBar));
    Assertions.assertTrue(TestUtility.activityStartingAtTime(plan, t2hr, actTypeBasic));
  ...
  public void testOrWithBacktrack() {
    ...
    final var exclusionBasic =
        TestUtility.createExclusionSchedulingZone(
            actTypeBasic, new Windows(Interval.between(t1hr.minus(d1min), t1hr.plus(d1min)), true));
    final var exclusionBar =
        TestUtility.createExclusionSchedulingZone(
            actTypeBar, new Windows(Interval.between(t1hr.minus(d1min), t1hr.plus(d1min)), true));
    problem.add(exclusionBasic);
    problem.add(exclusionBar);
    problem.setGoals(List.of(orGoal));
    final var solver = new PrioritySolver(problem);
  ...
  public void testCardinalityBacktrack() {
    ...
    CardinalityGoal unsatisfiableGoal =
        new CardinalityGoal.Builder()
            .duration(
                Interval.between(
                    Duration.of(16, Duration.SECONDS), Duration.of(21, Duration.SECONDS)))
