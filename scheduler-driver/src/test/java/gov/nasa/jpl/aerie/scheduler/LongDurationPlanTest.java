...
public class LongDurationPlanTest {
  ...
  private static Problem makeTestMissionAB() {
    ...
    return new Problem(
        banananationMissionModel,
        h,
        new SimulationFacade(h, banananationMissionModel),
  ...
  private static final PlanningHorizon h =
      new PlanningHorizon(
          ...
          TimeUtility.fromDOY("2030-005T01:01:01.001"));
  private static final Duration t0 = h.getStartAerie();
  private static final Duration d1year =
      Duration.of(1, Duration.SECONDS).times(3600).times(24).times(365);
  private static final Duration d1min = Duration.of(1, Duration.MINUTES);

  private static final Duration t1year = t0.plus(d1year);
  private static final Duration t2year = t1year.plus(d1year);

  private static final Duration t3year = t2year.plus(d1year);
  ...
  private static boolean equalsExceptInName(
      SchedulingActivityDirective a, SchedulingActivityDirective b) {
  ...
  private static final Correspondence<SchedulingActivityDirective, SchedulingActivityDirective>
      equalExceptInName = Correspondence.from(LongDurationPlanTest::equalsExceptInName, "matches");
