...
public record MissionModelGenerator(Elements elementUtils, Types typeUtils, Messager messager) {
  ...
  private static MethodSpec generateGetDirectiveCompoundTypes(List<ActivityTypeRecord> activityTypeRecords){
    final var method = MethodSpec
        .methodBuilder("getDirectiveCompoundTypes")
        .addModifiers(Modifier.PUBLIC)
        .addAnnotation(Override.class)
        .returns(
            ParameterizedTypeName.get(
                ClassName.get(Map.class),
                ClassName.get(String.class),
                ParameterizedTypeName.get(ClassName.get(Optional.class), ClassName.get(Boolean.class))));
    final var all = new ArrayList<String>();
    for(final var activityType : activityTypeRecords) {
      ...
      all.add("Map.entry(\"" + activityType.name() + "\"" + ',' + value+")");
    }
    method.addCode("return Map.ofEntries("  + String.join(",\n", all) + ");");
    return method.build();
  }
