...
public class AutoValueMappers {
  ...
  static TypeRule typeRule(final Element autoValueMapperElement, final ClassName generatedClassName)
    ...
    if (!autoValueMapperElement.getKind().equals(ElementKind.RECORD)) {
      throw new InvalidMissionModelException(
          "@%s.%s is only allowed on records"
              .formatted(
  ...
  static JavaFile generateAutoValueMappers(
      final MissionModelRecord missionModel, final Iterable<? extends Element> recordTypes) {
    ...
    for (final var record : recordTypes) {
      ...
      for (final var element : record.getEnclosedElements()) {
        ...
        final var valueMapperIdentifier =
            JAVA_IDENTIFIER_ILLEGAL_CHARACTERS.matcher(typeMirror.toString()).replaceAll("_")
                + "ValueMapper";
      ...
      builder.addMethod(
          MethodSpec.methodBuilder(methodName)
              ...
              .addTypeVariables(
                  ((TypeElement) record)
                      .getTypeParameters().stream().map(TypeVariableName::get).toList())
              .returns(
                  ParameterizedTypeName.get(
                      ClassName.get(ValueMapper.class), TypeName.get(record.asType())))
              .addParameters(
                  necessaryMappers.entrySet().stream()
                      .map(
                          mapperRequest ->
              ...
              .addCode(
                  CodeBlock.builder()
                      ...
                      .add(
                          "$L$T.class,\n",
                          // SAFETY: This cast cannot fail - it merely exists to compensate for an
                          // inability
                          // to express a MyRecord<T>.class. Instead, we use MyRecord.class, and
                          // cast it, via
                          // Object, to a `Class<MyRecord<T>>`. We need to go via Object because
                          // Java generics
                          // do not allow casting a Foo<Bar> to a Foo<Bar<Baz>> - since Bar may be a
                          // container
                          // that already contains non-Baz objects. In this case, since
                          // MyRecord.class is not
    ...
    return JavaFile.builder(typeName.packageName(), builder.build())
