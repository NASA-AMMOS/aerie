...
public abstract sealed class MapperMethodMaker
  ...
  public /*non-final*/ MethodSpec makeGetParametersMethod() {
    return MethodSpec.methodBuilder("getParameters")
        ...
        .returns(ParameterizedTypeName.get(java.util.ArrayList.class, Parameter.class))
        .addStatement(
            "final var $L = new $T()",
            "parameters",
            ParameterizedTypeName.get(java.util.ArrayList.class, Parameter.class))
        .addCode(
            inputType.parameters().stream()
                ...
                .build())
        .addStatement("return $L", "parameters")
        .build();
  }
  ...
  public final MethodSpec makeGetRequiredParametersMethod() {
    ...
    final var requiredParams =
        inputType.parameters().stream().filter(p -> !optionalParams.contains(p.name)).toList();

    return MethodSpec.methodBuilder("getRequiredParameters")
        .addModifiers(Modifier.PUBLIC)
        .addAnnotation(Override.class)
        .returns(ParameterizedTypeName.get(java.util.List.class, String.class))
        .addStatement(
            "return $T.of($L)",
            List.class,
  ...
  public final MethodSpec makeGetValidationFailuresMethod() {
    return MethodSpec.methodBuilder("getValidationFailures")
        .addModifiers(Modifier.PUBLIC)
        .addAnnotation(Override.class)
        .returns(ParameterizedTypeName.get(java.util.List.class, ValidationNotice.class))
        .addParameter(TypeName.get(inputType.declaration().asType()), "input", Modifier.FINAL)
        .addStatement(
            "final var $L = new $T()",
            "notices",
            ParameterizedTypeName.get(java.util.ArrayList.class, ValidationNotice.class))
        .addCode(
            inputType.validations().stream()
                .map(
                    validation -> {
                      final var subjects =
                          Arrays.stream(validation.subjects())
                ...
                .reduce(CodeBlock.builder(), (x, y) -> x.add(y.build()))
                .build())
        .addStatement("return $L", "notices")
        .build();
  }

  protected final MethodSpec.Builder makeArgumentAssignments(
      final MethodSpec.Builder methodBuilder,
      final BiFunction<CodeBlock.Builder, ParameterRecord, CodeBlock.Builder>
    ...
    mb =
        mb.beginControlFlow("switch ($L.getKey())", "entry")
            .addCode(
                inputType.parameters().stream()
                    .map(
                        parameter -> {
                          final var caseBuilder =
                              CodeBlock.builder().add("case $S:\n", parameter.name).indent();
                          return makeArgumentAssignment
                              .apply(caseBuilder, parameter)
                              .addStatement("break")
                              .unindent();
                        })
                    .reduce(CodeBlock.builder(), (x, y) -> x.add(y.build()))
            ...
            .addCode(
                CodeBlock.builder()
                    ...
                    .indent()
                    .addStatement(
                        "$L.withExtraneousArgument($L.getKey())", "instantiationExBuilder", "entry")
                    .unindent()
                    .build())
            .endControlFlow();
    ...
    if (shouldExpectArguments) {
      mb =
          mb.nextControlFlow("catch (final $T e)", UnconstructableArgumentException.class)
              .addStatement(
            ...
            inputType.parameters().stream()
                .map(
                    parameter ->
                        CodeBlock.builder()
                            .addStatement(
                                // Re-serialize value since provided `arguments` map may not contain
                                // the value (when using `@WithDefaults` templates)
                                "$L.ifPresentOrElse($Wvalue -> $L.withValidArgument(\"$L\","
                                    + " this.mapper_$L.serializeValue(value)),$W() ->"
                                    + " $L.withMissingArgument(\"$L\","
                                    + " this.mapper_$L.getValueSchema()))",
                                parameter.name,
                                "instantiationExBuilder",
                                parameter.name,
                                parameter.name,
                ...
                .build())
        .addCode("\n")
        .addStatement("$L.throwIfAny()", "instantiationExBuilder");
  }
