...
> = opts => async keys => {
  ...
  return Promise.all(keys.map(async ({ simulationDatasetId }) => {
    const simulationDataset = simulation_dataset.find(simulationDataset => simulationDataset.id === simulationDatasetId);
...
export interface ActivityAttributes<ActivityArguments extends {[attributeName: string]: unknown}, ActivityComputedAttributes> {
  arguments: ActivityArguments,
  directiveId: number,
  computedAttributes: ActivityComputedAttributes,
}

export interface SimulatedActivityInstance<ActivityArguments extends {[attributeName: string]: unknown}, ActivityComputedAttributes> {
  id: number,
