...
> = opts => async keys => {
  ...
  return Promise.all(keys.map(async ({ expansionSetId }) => {
    ...
    const commandTypes = await fs.promises.readFile(expansionSet.command_dictionary.command_types_typescript_path, 'utf8');
    return {
      id: expansionSet.id,
      commandDictionary: {
