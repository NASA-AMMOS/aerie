...
function generateTypescriptCodeForCommandDictionary(dictionary: ampcs.CommandDictionary): {
  declarations: string;
  values: string;
} {
...
export async function processDictionary(dictionary: ampcs.CommandDictionary) {
  const prefaceUrl = new URL('./CommandEDSLPreamble.js', import.meta.url);
  ...
  const { values, declarations } = generateTypescriptCodeForCommandDictionary(dictionary); //update sql table store separate
