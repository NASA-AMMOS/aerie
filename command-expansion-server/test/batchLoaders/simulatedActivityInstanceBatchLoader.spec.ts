...
import { simulatedActivitiesBatchLoader, simulatedActivityInstanceBySimulatedActivityIdBatchLoader } from '../../src/lib/batchLoaders/simulatedActivityBatchLoader.js';
...
it('should load simulated activity instance for simulation_dataset and simulated activity id', async () => {
  ...
  const simulatedActivityId = await convertActivityIdToSimulatedActivityId(graphqlClient, simulationArtifactIds.simulationDatasetId, activityId);

  const activityInstances = await simulatedActivityInstanceBySimulatedActivityIdBatchLoader({graphqlClient, activitySchemaDataLoader})([
