...
public record GenerateSchedulingLibAction(MissionModelService missionModelService) {
  ...
  public Response run(final MissionModelId missionModelId) {
    try {
      ...
      final var temporalPolyfillTypes =
          getTypescriptResource("constraints/TemporalPolyfillTypes.ts");

      final var missionModelTypes = missionModelService.getMissionModelTypes(missionModelId);

      final var generatedSchedulerCode =
          TypescriptCodeGenerationService.generateTypescriptTypesFromMissionModel(
              missionModelTypes);
      final var generatedConstraintsCode =
          gov.nasa.jpl.aerie.constraints.TypescriptCodeGenerationService.generateTypescriptTypes(
      ...
      return new Response.Success(
          Map.of(
              "file:///%s".formatted(schedulingDsl.basename),
              schedulingDsl.source,
              "file:///scheduler-mission-model-generated-code.ts",
              generatedSchedulerCode,
              "file:///%s".formatted(schedulerAst.basename),
              schedulerAst.source,
              "file:///%s".formatted(windowsDsl.basename),
              windowsDsl.source,
              "file:///%s".formatted(windowsAst.basename),
              windowsAst.source,
              "file:///mission-model-generated-code.ts",
              generatedConstraintsCode,
              "file:///%s".formatted(temporalPolyfillTypes.basename),
              temporalPolyfillTypes.source));
    } catch (final NoSuchMissionModelException
        | IOException
        | MissionModelService.MissionModelServiceException e) {
  ...
  /*package-private*/ static TypescriptResource getTypescriptResource(final String basename) {
    ...
    if (stream == null) throw new Error("Resource path does not exist: `/%s`".formatted(basename));
    final var source =
