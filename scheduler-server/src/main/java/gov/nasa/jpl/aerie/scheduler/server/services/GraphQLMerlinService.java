 ...
 */
public record GraphQLMerlinService(URI merlinGraphqlURI)
    implements PlanService.OwnerRole, MissionModelService {
   ...
   */
  protected Optional<JsonObject> postRequest(final String gqlStr)
    ...
    try {
      ...
      final var httpReq =
          HttpRequest.newBuilder()
              ...
              .build();
      // TODO: (net optimization) gzip compress the request body if large enough (eg for
      // createAllActs)
      final var httpResp =
          HttpClient.newHttpClient().send(httpReq, HttpResponse.BodyHandlers.ofInputStream());
      if (httpResp.statusCode() != 200) {
        // TODO: how severely to error out if aerie cannot be reached or has a 500 error or json is
        // garbled etc etc?
      ...
      // TODO: maybe retry if interrupted? but depends on semantics (eg don't duplicate mutation if
      // not idempotent)
      return Optional.empty();
  ...
  // TODO: maybe use fancy aerie typed json parsers/serializers, ala BasicParsers.productP use in
  // MerlinParsers
  ...
  public long getPlanRevision(final PlanId planId)
    ...
    final var response =
        postRequest(query, variables).orElseThrow(() -> new NoSuchPlanException(planId));
    try {
      return response
          .getJsonObject("data")
          .getJsonObject("plan_by_pk")
          .getJsonNumber("revision")
          .longValueExact();
  ...
  public PlanMetadata getPlanMetadata(final PlanId planId)
    ...
    try {
      // TODO: elevate and then leverage existing MerlinParsers (after updating them to match
      // current db!)
      ...
      // NB: not using the "path" field because it is just a hex-encoded duplicate of the name field
      // anyway
      // NB: the name includes the .jar extension

      // TODO: how to know right model config for scheduling? for now choosing latest sim setup (see
      // query above)
      ...
      if (!sims.isEmpty()) {
        ...
        modelConfiguration =
            BasicParsers.mapP(serializedValueP)
                .parse(args)
                .getSuccessOrThrow(
                    (reason) ->
                        new InvalidJsonException(new InvalidEntityException(List.of(reason))));
      }
      ...
      // TODO: better error reporting upward to service response (NSPEx doesn't allow passing e as
      // cause)
      throw new NoSuchPlanException(planId);
    }
  }
  ...
  public MerlinPlan getPlanActivityDirectives(
      final PlanMetadata planMetadata, final Problem problem)
      throws IOException,
          NoSuchPlanException,
    ...
    final var request =
        "query { plan_by_pk(id:%d) { activity_directives { id start_offset type arguments anchor_id anchored_to_start } duration start_time }} "
            .formatted(planMetadata.planId().id());
    final var response =
        postRequest(request).orElseThrow(() -> new NoSuchPlanException(planMetadata.planId()));
    ...
    for (int i = 0; i < activityDirectives.size(); i++) {
      ...
      final Integer anchorId =
          jsonActivity.isNull("anchor_id") ? null : jsonActivity.getInt("anchor_id");
      ...
      final var deserializedArguments =
          BasicParsers.mapP(serializedValueP)
              .parse(arguments)
              .getSuccessOrThrow(
                  (reason) ->
      ...
      final var merlinActivity =
          new ActivityDirective(
              Duration.of(
                  parseGraphQLInterval(start).getDuration().toNanos() / 1000,
  ...
  public String getNextPlanName() {
    // TODO: (defensive) should rely on database to generate a new unique name to avoid user
    // collisions
  ...
  public Pair<PlanId, Map<SchedulingActivityDirective, ActivityDirectiveId>>
    ...
    final var planId =
        createEmptyPlan(
            ...
            planMetadata.horizon().getEndAerie());
    final Map<SchedulingActivityDirective, ActivityDirectiveId> activityToId =
        createAllPlanActivityDirectives(planId, plan, activityToGoalId);
  ...
  public PlanId createEmptyPlan(
    ...
    // NB: the duration format for creating plans is different than that for activity instances
    // (microseconds)
    final var durStr = "\"" + duration.in(Duration.SECOND) + "\"";
    final var request =
        requestFormat.formatted(
  ...
  public Map<SchedulingActivityDirective, ActivityDirectiveId> updatePlanActivityDirectives(
    ...
    for (final var activity : plan.getActivities()) {
      ...
      if (idActFromInitialPlan != null) {
        ...
        if (activity.getType().getDurationType()
            instanceof DurationType.Controllable durationType) {
          if (!activity.arguments().containsKey(durationType.parameterName())) {
            activity.addArgument(
        ...
        final var activityDirectiveFromSchedulingDirective =
            new ActivityDirective(
        ...
        if (!activityDirectiveFromSchedulingDirective.equals(actFromInitialPlan.get())) {
          ...
          // updateActivityDirective(planId, schedulerActIntoMerlinAct, activityDirectiveId,
          // activityToGoalId.get(activity));
        }
  ...
  public void ensurePlanExists(final PlanId planId)
    ...
    final var request =
        "query ensurePlanExists { plan_by_pk( id: %s ) { id } }".formatted(planId.id());
  ...
  public void clearPlanActivityDirectives(final PlanId planId)
    ...
    try {
      response
          .getJsonObject("data")
          .getJsonObject("delete_activity")
          .getJsonNumber("affected_rows")
          .longValueExact();
    } catch (ClassCastException | ArithmeticException e) {
      throw new NoSuchPlanException(planId);
    }
  ...
  public Map<SchedulingActivityDirective, ActivityDirectiveId> createActivityDirectives(
    ...
    // TODO: (optimization) could use a lazy evaluating stream of strings to avoid large set of
    // strings in memory
    ...
    for (final var act : orderedActivities) {
      ...
      if (act.getType().getDurationType() instanceof DurationType.Controllable durationType) {
        if (!act.arguments().containsKey(durationType.parameterName())) {
          insertionObjectArguments.add(
              durationType.parameterName(), act.duration().in(Duration.MICROSECOND));
        }
      }
      ...
      for (final var arg : act.arguments().entrySet()) {
        // serializedValueP is safe to use here because only unparsing. otherwise subject to
        // int/double typing confusion
    ...
    final var arguments =
        Json.createObjectBuilder().add("activities", insertionObjects.build()).build();

    final var response =
        postRequest(query, arguments).orElseThrow(() -> new NoSuchPlanException(planId));

    final Map<SchedulingActivityDirective, ActivityDirectiveId> instanceToInstanceId =
        new HashMap<>();
    try {
      final var numCreated =
          response
              .getJsonObject("data")
              .getJsonObject("insert_activity_directive")
              .getJsonNumber("affected_rows")
              .longValueExact();
      ...
      for (int i = 0; i < ids.size(); i++) {
        instanceToInstanceId.put(
            orderedActivities.get(i), new ActivityDirectiveId(ids.getJsonObject(i).getInt("id")));
      }
    } catch (ClassCastException | ArithmeticException e) {
      throw new NoSuchPlanException(planId);
    }
  ...
  public MissionModelTypes getMissionModelTypes(final PlanId planId)
    ...
    try {
      ...
      throw new MissionModelServiceException(
          "Failed to get mission model types for plan id %s".formatted(planId), e);
    }
    ...
    for (final var activityTypeJson : activityTypesJsonArray) {
      ...
      for (final var parameterJson : parametersJson.entrySet()) {
        parameters.put(
            ...
            valueSchemaP
                .parse(parameterJson.getValue().asJsonObject().getJsonObject("schema"))
                .getSuccessOrThrow());
      }
      ...
      for (final var presetJson : presetsJsonArray) {
        ...
        presets.put(presetJson.asJsonObject().getString("name"), arguments);
      }
      activityTypes.add(
          new ActivityType(activityTypeJson.asJsonObject().getString("name"), parameters, presets));
    }
  ...
  public MissionModelTypes getMissionModelTypes(final MissionModelId missionModelId)
    ...
    try {
      ...
      throw new MissionModelServiceException(
          "Failed to get mission model types for model id %s".formatted(missionModelId), e);
    }
    final var data = response.getJsonObject("data");
    if (data.get("mission_model_by_pk").getValueType().equals(JsonValue.ValueType.NULL))
      throw new NoSuchMissionModelException(missionModelId);
    final var activityTypesJsonArray =
        data.getJsonObject("mission_model_by_pk").getJsonArray("activity_types");
    final var activityTypes = parseActivityTypes(activityTypesJsonArray);
  ...
  public DatasetId storeSimulationResults(
    ...
    final var profileRecords =
        postResourceProfiles(
            datasetIds.datasetId(), profileSet.realProfiles(), profileSet.discreteProfiles());
    postProfileSegments(datasetIds.datasetId(), profileRecords, profileSet);
    postActivities(
        datasetIds.datasetId(),
        results.simulatedActivities,
        results.unfinishedActivities,
        results.startTime,
        simulationActivityDirectiveIdToMerlinActivityDirectiveId);
  ...
  private SimulationId createSimulation(
      final PlanId planId, final Map<String, SerializedValue> arguments)
      throws PlanServiceException, IOException {
    final var request =
        """
        mutation {
          insert_simulation_one(object: {plan_id: %d, arguments: %s}) {
            ...
            .formatted(planId.id(), simulationArgumentsP.unparse(arguments));
    final JsonObject response;
    response = postRequest(request).get();
    final var data = response.getJsonObject("data");
  ...
  private void setSimulationDatasetStatus(SimulationDatasetId id, SimulationStateRecord state)
    ...
    final var request =
        ...
        """
            .formatted(id.id(), state.status().label);
    final JsonObject response;
    response = postRequest(request).get();
    final var data = response.getJsonObject("data");
    ...
    if (affected != 1) {
      throw new PlanServiceException(
          "Unable to modify the status of simulation dataset with id %d".formatted(id.id()));
    }
  }
  ...
  private DatasetIds createSimulationDataset(SimulationId simulationId, PlanMetadata planMetadata)
    ...
    final var simulationDatasetId =
        data.getJsonObject("insert_simulation_dataset_one").getInt("id");
  ...
  private HashMap<String, ProfileRecord> postResourceProfiles(
    ...
    final var arguments = Json.createObjectBuilder().add("profiles", allProfiles.build()).build();
    final JsonObject response;
    response = postRequest(req, arguments).get();
    final var data =
        response.getJsonObject("data").getJsonObject("insert_profile").getJsonArray("returning");
    ...
    for (int i = 0; i < resourceNames.size(); i++) {
      ...
      profileRecords.put(resource, new ProfileRecord(id, datasetId.id(), resource, type, duration));
    }
  ...
  public com.impossibl.postgres.api.data.Interval graphQLIntervalFromDuration(
    ...
    for (final var entry : records.entrySet()) {
      ...
      switch (record.type().getLeft()) {
        case "real" -> postRealProfileSegments(
            datasetId, record, realProfiles.get(resource).getRight());
        case "discrete" -> postDiscreteProfileSegments(
            datasetId, record, discreteProfiles.get(resource).getRight());
  ...
  private <Dynamics> void postProfileSegment(
    ...
    final var arguments = Json.createObjectBuilder().add("profileSegments", profiles).build();
    ...
    final var affected_rows =
        response
            .getJsonObject("data")
            .getJsonObject("insert_profile_segment")
  ...
  private void postRealProfileSegments(
      final DatasetId datasetId,
      final ProfileRecord profileRecord,
  ...
  private void postDiscreteProfileSegments(
      final DatasetId datasetId,
      final ProfileRecord profileRecord,
  ...
  private void insertSimulationTopics(
      DatasetId datasetId, final List<Triple<Integer, String, ValueSchema>> topics)
      throws PlanServiceException, IOException {
    final var req =
        """
    ...
    final var arguments = Json.createObjectBuilder().add("topics", jsonTopics.build()).build();
    postRequest(req, arguments);
  }

  private void insertSimulationEvents(
      DatasetId datasetId,
      Map<Duration, List<EventGraph<Pair<Integer, SerializedValue>>>> eventPoints)
      throws PlanServiceException, IOException {
    final var req =
        """
    ...
    for (final var eventPoint : eventPoints.entrySet()) {
      ...
      for (int transactionIndex = 0; transactionIndex < transactions.size(); transactionIndex++) {
        ...
        events.addAll(
            batchInsertEventGraph(datasetId.id(), time, transactionIndex, flattenedEventGraph));
      }
    }
    final var arguments = Json.createObjectBuilder().add("events", events).build();
    postRequest(req, arguments);
  }
  ...
  private void postActivities(
      ...
      final Map<ActivityDirectiveId, ActivityDirectiveId>
          simulationActivityDirectiveIdToMerlinActivityDirectiveId)
      throws PlanServiceException, IOException {
    final var simulatedActivityRecords =
        simulatedActivities.entrySet().stream()
            .collect(
                Collectors.toMap(
                    e -> e.getKey().id(),
                    e ->
                        simulatedActivityToRecord(
                            e.getValue(),
                            simulationActivityDirectiveIdToMerlinActivityDirectiveId)));
    final var allActivityRecords =
        unfinishedActivities.entrySet().stream()
            .collect(
                Collectors.toMap(
                    e -> e.getKey().id(),
                    e ->
                        unfinishedActivityToRecord(
                            e.getValue(),
                            simulationActivityDirectiveIdToMerlinActivityDirectiveId)));
    allActivityRecords.putAll(simulatedActivityRecords);
    final var simIdToPgId = postSpans(datasetId, allActivityRecords, simulationStart);
    updateSimulatedActivityParentsAction(datasetId, simulatedActivityRecords, simIdToPgId);
  }
  ...
  public void updateSimulatedActivityParentsAction(
          ...
          Json.createObjectBuilder()
              .add(
                  ...
                  Json.createObjectBuilder()
                      .add(
                          "dataset_id",
                          Json.createObjectBuilder().add("_eq", datasetId.id()).build())
                      .add(
                          "id", Json.createObjectBuilder().add("_eq", simIdToPgId.get(id)).build()))
              .add(
                  "_set",
                  Json.createObjectBuilder()
                      .add("parent_id", simIdToPgId.get(activity.parentId().get())))
              .build());
      updateCounter++;
    }
    final var arguments = Json.createObjectBuilder().add("updates", updates.build()).build();
    ...
    if (jsonValue.getValueType() == JsonValue.ValueType.ARRAY) {
      for (final var jsonObject :
          response
              .getJsonObject("data")
              .getJsonArray("update_span_many")
              .getValuesAs(JsonObject.class)) {
        affected_rows += jsonObject.getInt("affected_rows");
      }
    } else {
      affected_rows =
          response.getJsonObject("data").getJsonObject("update_span_many").getInt("affected_rows");
    }
  ...
  private static SpanRecord simulatedActivityToRecord(
      final SimulatedActivity activity,
      final Map<ActivityDirectiveId, ActivityDirectiveId>
          simulationActivityDirectiveIdToMerlinActivityDirectiveId) {
    return new SpanRecord(
        ...
        new ActivityAttributesRecord(
            activity
                .directiveId()
                .map(id -> simulationActivityDirectiveIdToMerlinActivityDirectiveId.get(id).id()),
            activity.arguments(),
            Optional.of(activity.computedAttributes())));
  }

  private static SpanRecord unfinishedActivityToRecord(
      final UnfinishedActivity activity,
      final Map<ActivityDirectiveId, ActivityDirectiveId>
          simulationActivityDirectiveIdToMerlinActivityDirectiveId) {
    return new SpanRecord(
        activity.type(),
  ...
  public HashMap<Long, Long> postSpans(
      final DatasetId datasetId, final Map<Long, SpanRecord> spans, final Instant simulationStart)
      throws PlanServiceException, IOException {
    final var req =
        """
    ...
    for (final var id : ids) {
      ...
      spansJson.add(
          Json.createObjectBuilder()
              ...
              .add(
                  "duration",
                  act.duration.isPresent()
                      ? graphQLIntervalFromDuration(act.duration().get()).toString()
                      : "null")
              .add("type", act.type())
              .add(
                  "attributes",
                  buildAttributes(
                      act.attributes().directiveId(),
                      act.attributes().arguments(),
                      act.attributes().computedAttributes()))
              .build());
    }
    final var arguments = Json.createObjectBuilder().add("spans", spansJson).build();
    final JsonObject response;
    response = postRequest(req, arguments).get();
    final var returnedIds =
        response.getJsonObject("data").getJsonObject("insert_span").getJsonArray("returning");
  ...
  private JsonValue buildAttributes(
    ...
    return activityAttributesP.unparse(
        new ActivityAttributesRecord(directiveId, arguments, returnValue));
  }
