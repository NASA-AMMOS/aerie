 ...
 */
// TODO: elevate to a merlin-level library to allow parsing in various modules (eg dupl in
// scheduler...DemuxJson)
public class GraphQLParsers {
  ...
  // TODO: inconsistent with DOY format in Timestamp.fromString, MerlinParsers.timestampP, etc used
  // in merlin-server
  ...
  public static final Pattern intervalPattern =
      Pattern.compile(
  ...
  public static Interval parseGraphQLInterval(final String in) {
    ...
    if (!matcher.matches()) {
      throw new DateTimeParseException(
          "unable to parse HH:MM:SS.sss duration from \"" + in + "\"", in, 0);
    }
    final var signValues = Map.of("+", 1, "-", -1);
    final var sign = Optional.ofNullable(matcher.group("sign")).map(signValues::get).orElse(1);
    final var hr =
        Optional.ofNullable(matcher.group("hr"))
            .map(Integer::parseInt)
            .map(Duration::ofHours)
            .orElse(Duration.ZERO);
    final var min =
        Optional.ofNullable(matcher.group("min"))
            .map(Integer::parseInt)
            .map(Duration::ofMinutes)
            .orElse(Duration.ZERO);
    final var sec =
        Optional.ofNullable(matcher.group("sec"))
  ...
  public static final JsonParser<RealDynamics> realDynamicsP =
      productP
          ...
          .field("rate", doubleP)
          .map(untuple(RealDynamics::linear), $ -> tuple($.initial, $.rate));
