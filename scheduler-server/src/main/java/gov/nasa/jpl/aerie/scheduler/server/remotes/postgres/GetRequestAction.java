...
/*package-local*/ final class GetRequestAction implements AutoCloseable {
  ...
  public Optional<RequestRecord> get(final long specificationId, final long specificationRevision)
      throws SQLException {
    ...
    try {
      ...
      throw new Error(
          String.format(
              "Scheduling request for specification with ID %d and revision %d has invalid state"
                  + " %s",
              specificationId, specificationRevision, ex.invalidStatus));
    }
