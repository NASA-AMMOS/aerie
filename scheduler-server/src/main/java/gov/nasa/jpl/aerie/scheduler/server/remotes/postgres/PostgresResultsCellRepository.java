...
public final class PostgresResultsCellRepository implements ResultsCellRepository {
  ...
  private static SpecificationRecord getSpecification(
      final Connection connection, final SpecificationId specificationId)
  ...
  private static Optional<RequestRecord> getRequest(
    ...
    try (final var getRequestAction = new GetRequestAction(connection)) {
      return getRequestAction.get(specificationId.id(), specificationRevision);
    }
  }

  private static RequestRecord createRequest(
      final Connection connection, final SpecificationRecord specification) throws SQLException {
  ...
  private static void deleteRequest(
      final Connection connection, final SpecificationId specId, final long specRevision)
      throws SQLException {
  ...
  private static void failRequest(
    ...
    try (final var setRequestStateAction = new SetRequestStateAction(connection)) {
      setRequestStateAction.apply(
          specId.id(), specRevision, RequestRecord.Status.FAILED, reason, Optional.empty());
    }
  }
  ...
  private static void succeedRequest(
    ...
    try (final var setRequestStateAction = new SetRequestStateAction(connection)) {
      setRequestStateAction.apply(
          specId.id(), specRevision, RequestRecord.Status.SUCCESS, null, datasetId);
    }
  }

  private static void postResults(
      final Connection connection, final long analysisId, final ScheduleResults results)
      throws SQLException {
    ...
    try (final var insertGoalSatisfactionAction = new InsertGoalSatisfactionAction(connection);
        final var insertCreatedActivitiesAction = new InsertCreatedActivitiesAction(connection);
        final var insertSatisfyingActivitiesAction =
            new InsertSatisfyingActivitiesAction(connection)) {
  ...
  private static ScheduleResults getResults(final Connection connection, final long analysisId)
      throws SQLException {
    try (final var getGoalSatisfactionAction = new GetGoalSatisfactionAction(connection);
  ...
  public static final class PostgresResultsCell implements ResultsProtocol.OwnerRole {
    ...
    public ResultsProtocol.State get() {
      try (final var connection = dataSource.getConnection()) {
        return getRequestState(connection, specId, specRevision)
