...
public class SchedulingDSL {
  ...
  private static final JsonParser<ClosedOpenInterval> intervalP =
      productP
          ...
          .field("end", durationP)
          .map(untuple(ClosedOpenInterval::new), $ -> tuple($.start(), $.end()));
  ...
  private static final JsonParser<CardinalitySpecification> cardinalitySpecificationJsonParser =
      productP
          ...
          .map(untuple(CardinalitySpecification::new), $ -> tuple($.duration(), $.occurrence()));
  ...
  private static final JsonObjectParser<ConstraintExpression.ActivityExpression>
      activityExpressionP =
          productP
              ...
              .map(
                  $ ->
                      new ConstraintExpression.ActivityExpression(
  ...
  private static final JsonParser<LinearResource> linearResourceP =
      stringP.map(LinearResource::new, LinearResource::name);
  ...
  private static final JsonObjectParser<GoalSpecifier.CoexistenceGoalDefinition>
      coexistenceGoalDefinitionP(MissionModelService.MissionModelTypes activityTypes) {
    return productP
        .field("activityTemplate", new ActivityTemplateJsonParser(activityTypes))
        .optionalField("activityFinder", activityExpressionP)
        .field("alias", stringP)
  ...
  private static final JsonObjectParser<GoalSpecifier.CardinalityGoalDefinition>
      cardinalityGoalDefinitionP(MissionModelService.MissionModelTypes activityTypes) {
    return productP
        .field("activityTemplate", new ActivityTemplateJsonParser(activityTypes))
        .optionalField("activityFinder", activityExpressionP)
  ...
  private static JsonObjectParser<GoalSpecifier.GoalAnd> goalAndF(
      final JsonParser<GoalSpecifier> goalSpecifierP) {
    return productP
        .field("goals", listP(goalSpecifierP))
        .field("shouldRollbackIfUnsatisfied", boolP)
        .map(
            untuple(GoalSpecifier.GoalAnd::new),
            goalDefinition ->
                tuple(goalDefinition.goals, goalDefinition.shouldRollbackIfUnsatisfied));
  }

  private static JsonObjectParser<GoalSpecifier.GoalOr> goalOrF(
      final JsonParser<GoalSpecifier> goalSpecifierP) {
    return productP
        .field("goals", listP(goalSpecifierP))
        .field("shouldRollbackIfUnsatisfied", boolP)
        .map(
            untuple(GoalSpecifier.GoalOr::new),
            goalDefinition ->
                tuple(goalDefinition.goals, goalDefinition.shouldRollbackIfUnsatisfied));
  }

  private static JsonObjectParser<GoalSpecifier.GoalApplyWhen> goalApplyWhenF(
      final JsonParser<GoalSpecifier> goalSpecifierP) {
    return productP
        .field("goal", goalSpecifierP)
        .field("window", windowsExpressionP)
        .map(
            untuple(GoalSpecifier.GoalApplyWhen::new),
            goalDefinition -> tuple(goalDefinition.goal(), goalDefinition.windows()));
  }

  private static JsonParser<GoalSpecifier> goalSpecifierF(
      MissionModelService.MissionModelTypes missionModelTypes) {
    return recursiveP(
        self ->
            SumParsers.sumP(
                "kind",
                ...
                List.of(
                    ...
                    SumParsers.variant(
                        "ApplyWhen", GoalSpecifier.GoalApplyWhen.class, goalApplyWhenF(self)))));
  }

  private static final JsonObjectParser<ConditionSpecifier.GlobalSchedulingCondition>
  ...
  private static JsonObjectParser<ConditionSpecifier.AndCondition> conditionAndF(
      final JsonParser<ConditionSpecifier> conditionSpecifierP) {
    return productP
        .field("conditions", listP(conditionSpecifierP))
        .map(
            untuple(ConditionSpecifier.AndCondition::new),
  ...
  public static final JsonParser<GoalSpecifier> schedulingJsonP(
      MissionModelService.MissionModelTypes missionModelTypes) {
    return goalSpecifierF(missionModelTypes);
  }

  public sealed interface ConditionSpecifier {
    record GlobalSchedulingCondition(Expression<Windows> expression, List<String> activityTypes)
        implements ConditionSpecifier {}

    record AndCondition(List<ConditionSpecifier> conditionSpecifiers)
        implements ConditionSpecifier {}
  }
  ...
  public sealed interface GoalSpecifier {
    ...
    record GoalAnd(List<GoalSpecifier> goals, boolean shouldRollbackIfUnsatisfied)
        implements GoalSpecifier {}

    record GoalOr(List<GoalSpecifier> goals, boolean shouldRollbackIfUnsatisfied)
        implements GoalSpecifier {}

    record GoalApplyWhen(GoalSpecifier goal, Expression<Windows> windows)
        implements GoalSpecifier {}
  }

  public record LinearResource(String name) {}

  public record CardinalitySpecification(
      Optional<Duration> duration, Optional<Integer> occurrence) {}
