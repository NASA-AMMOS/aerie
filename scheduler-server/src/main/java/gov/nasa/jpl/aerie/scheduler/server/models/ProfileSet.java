...
public record ProfileSet(
    ...
    Map<String, Pair<ValueSchema, List<ProfileSegment<Optional<SerializedValue>>>>>
        discreteProfiles) {
  ...
  public static ProfileSet of(
      ...
      final Map<String, Pair<ValueSchema, List<ProfileSegment<SerializedValue>>>>
          discreteProfiles) {
    return new ProfileSet(wrapInOptional(realProfiles), wrapInOptional(discreteProfiles));
  }

  public static <T>
      Map<String, Pair<ValueSchema, List<ProfileSegment<Optional<T>>>>> wrapInOptional(
        ...
        .map(
            $ ->
                Pair.of(
                    ...
                    Pair.of(
                        $.getValue().getLeft(),
                        $.getValue().getRight().stream()
                            .map(
                                untuple(
                                    segment ->
                                        new ProfileSegment<>(
                                            segment.extent(), Optional.of(segment.dynamics()))))
                            .toList())))
                    ...
                    Pair.of(
                        ...
                        $.getValue().getRight().stream()
                            .map(
                                segment ->
                                    new ProfileSegment<>(
                                        segment.extent(), segment.dynamics().get()))
                            .toList())))
        .collect(Collectors.toMap(Pair::getKey, Pair::getValue));
  }
}
