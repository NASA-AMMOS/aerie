...
public final class PreparedStatements {
  ...
  public static void setFailureReason(
      final PreparedStatement statement, final int parameter, final ScheduleFailure reason)
      throws SQLException {
    statement.setString(
        parameter,
        reason == null ? null : SchedulerParsers.scheduleFailureP.unparse(reason).toString());
  }

  public static Optional<ScheduleFailure> getFailureReason(
      final ResultSet results, final String columnLabel) throws SQLException {
    final var failureJson = results.getString(columnLabel);
    return failureJson == null || failureJson.isBlank()
        ? Optional.empty()
        : Optional.of(
            PreparedStatements.deserializeScheduleFailure(results.getString(columnLabel)));
  }
  ...
  public static Optional<Long> getDatasetId(final ResultSet results, final String columnLabel)
    ...
    return datasetId == null ? Optional.empty() : Optional.of(results.getLong(columnLabel));
  }
}
