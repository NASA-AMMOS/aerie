...
public final class DurationTest {
  ...
  public void testRoundDown() {
    ...
    assertEquals(
        duration(2756, MILLISECONDS).minus(EPSILON), roundDownward(Math.nextDown(2.756), SECONDS));
  }
  ...
  public void testRoundUp() {
    ...
    assertEquals(
        duration(2756, MILLISECONDS).plus(EPSILON), roundUpward(Math.nextUp(2.756), SECONDS));
  }
  ...
  public void testSaturation() {
    ...
    assertEquals(
        Duration.MAX_VALUE.minus(Duration.SECOND).saturatingPlus(Duration.of(2, SECONDS)),
        Duration.MAX_VALUE);
    assertEquals(Duration.MIN_VALUE.saturatingPlus(Duration.of(-1, SECONDS)), Duration.MIN_VALUE);
    assertEquals(
        Duration.MIN_VALUE.plus(Duration.SECOND).saturatingPlus(Duration.of(-2, SECONDS)),
        Duration.MIN_VALUE);
  }
}
