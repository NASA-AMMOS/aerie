...
Target = namedtuple('Target', ['monad_pattern', 'file_name'])
TARGETS = [
    Target('Resource<{}>', '../core/monads/ResourceMonad.java'),
    Target('ErrorCatching<Expiring<{}>>', '../core/monads/DynamicsMonad.java'),
    Target('Expiring<{}>', '../core/monads/ExpiringMonad.java'),
    Target('ErrorCatching<{}>', '../core/monads/ErrorCatchingMonad.java'),
    Target('ThinResource<{}>', '../core/monads/ThinResourceMonad.java'),
    Target('Discrete<{}>', '../modeling/discrete/monads/DiscreteMonad.java'),
    Target('Resource<Discrete<{}>>', '../modeling/discrete/monads/DiscreteResourceMonad.java'),
    Target('ErrorCatching<Expiring<Discrete<{}>>>', '../modeling/discrete/monads/DiscreteDynamicsMonad.java'),
]
...
def main(n):
    for target in TARGETS:
        ...
        supplemental_methods_content = generate_supplemental_methods(target.monad_pattern, n)
...
def generate_supplemental_methods(monad_pattern, n):
    result = f'// Supplemental methods generated by {os.path.basename(__file__)} on {datetime.date.today().isoformat()}.'
    result += '\n' + generate_functional_apply(monad_pattern)
    result += '\n' + generate_1_arg_map(monad_pattern)
    result += '\n' + generate_1_arg_functional_map(monad_pattern)
    result += '\n' + generate_1_arg_bind(monad_pattern)
    result += '\n' + generate_1_arg_functional_bind(monad_pattern)
    for i in range(2, n + 1):
        result += '\n' + generate_n_arg_map(i, monad_pattern)
        result += '\n' + generate_n_arg_curried_map(i, monad_pattern)
        result += '\n' + generate_n_arg_functional_map(i, monad_pattern)
        result += '\n' + generate_n_arg_bind(i, monad_pattern)
        result += '\n' + generate_n_arg_curried_bind(i, monad_pattern)
        result += '\n' + generate_n_arg_functional_bind(i, monad_pattern)
    return result
