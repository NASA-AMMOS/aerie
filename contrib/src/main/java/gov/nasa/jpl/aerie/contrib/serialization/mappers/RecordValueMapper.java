...
public final class RecordValueMapper<R extends Record> implements ValueMapper<R> {
  ...
  public RecordValueMapper(final Class<R> recordType, final List<Component<R, ?>> components) {
    this.recordType = recordType;
    this.components = components;
  }

  public record Component<R, T>(String name, Function<R, T> projection, ValueMapper<T> mapper) {}

  @Override
  public ValueSchema getValueSchema() {
    final var valueSchemas = new HashMap<String, ValueSchema>();
    for (final var component : this.components) {
      valueSchemas.put(component.name, component.mapper.getValueSchema());
    }
  ...
  public Result<R, String> deserializeValue(final SerializedValue serializedValue) {
    return serializedValue
        .asMap()
        .map(
            (Function<Map<String, SerializedValue>, Result<R, String>>)
                (map -> {
                  final var validKeys = new HashSet<String>();
                  for (final var component : components) {
                    if (!map.containsKey(component.name))
                      return Result.failure("Record missing key %s".formatted(component));
                    validKeys.add(component.name);
                  }
                  for (final var key : map.keySet()) {
                    if (!validKeys.contains(key))
                      return Result.failure("Record has extra key %s".formatted(key));
                  }
                  final var arguments = new Object[components.size()];
                  for (var i = 0; i < components.size(); i++) {
                    final var component = components.get(i);
                    final var deserializedValue =
                        component.mapper.deserializeValue(map.get(component.name));
                    if (deserializedValue.getKind() == Result.Kind.Failure)
                      return Result.failure("Failed to deserialize %s".formatted(component.name));
                    arguments[i] = deserializedValue.getSuccessOrThrow();
                  }
                  try {
                    return Result.success(
                        getCanonicalConstructor(this.recordType).newInstance(arguments));
  ...
  private static <R> Constructor<R> getCanonicalConstructor(final Class<R> cls)
