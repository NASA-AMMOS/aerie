...
/*package local*/ final class GetSimulationAction implements AutoCloseable {
  ...
  public SimulationRecord get(final long planId) throws SQLException {
    ...
    if (!results.next())
      throw new SQLException("Plan " + planId + " does not have a SimulationRecord.");
    ...
    final var arguments =
        getJsonColumn(results, "arguments", simulationArgumentsP)
            .getSuccessOrThrow(
                failureReason ->
                    new Error(
                        "Corrupt simulation arguments cannot be parsed: "
    ...
    return new SimulationRecord(
        id, revision, planId, templateId$, arguments, simulationStartTime, simulationEndTime);
  }
