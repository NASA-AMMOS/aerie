...
public final class MerlinBindings implements Plugin {
  ...
  private void postRefreshModelParameters(final Context ctx) {
    try {
      final var missionModelId =
          parseJson(ctx.body(), hasuraMissionModelEventTriggerP).missionModelId();
      ...
      ctx.status(404)
          .result(ResponseSerializers.serializeNoSuchMissionModelException(ex).toString());
    }
  }
  ...
  private void postRefreshActivityValidations(final Context ctx) {
    try {
      ...
      final var serializedActivity =
          new SerializedActivity(input.activityTypeName(), input.arguments());
      final var activityDirective =
          new ActivityDirectiveForValidation(
              input.activityDirectiveId(),
              input.planId(),
              input.argumentsModifiedTime(),
              serializedActivity);
  ...
  private void getResourceTypes(final Context ctx) {
    try {
      final var missionModelId =
          parseJson(ctx.body(), hasuraMissionModelActionP).input().missionModelId();
  ...
  private void getConstraintViolations(final Context ctx) {
    try {
      ...
      ctx.result(
          ResponseSerializers.serializeConstraintViolations(constraintViolations).toString());
    } catch (final InvalidJsonException ex) {
      ctx.status(400).result(ResponseSerializers.serializeInvalidJsonException(ex).toString());
    } catch (final InvalidEntityException ex) {
      ctx.status(400).result(ResponseSerializers.serializeInvalidEntityException(ex).toString());
  ...
  private void validateActivityArguments(final Context ctx) {
    try {
      ...
      final var notices =
          this.missionModelService.validateActivityArguments(missionModelId, serializedActivity);
  ...
  private void validateModelArguments(final Context ctx) {
    try {
      ...
      final var notices =
          this.missionModelService.validateModelArguments(missionModelId, arguments);
  ...
  private void validatePlan(final Context ctx) {
    try {
      ...
      final var activities =
          plan.activityDirectives.entrySet().stream()
              .collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().serializedActivity()));
      final var failures =
          this.missionModelService.validateActivityInstantiations(plan.missionModelId, activities);
  ...
  private void getModelEffectiveArguments(final Context ctx) {
    try {
      ...
      final var missionModelId = input.missionModelId();
      final var arguments =
          this.missionModelService.getModelEffectiveArguments(missionModelId, input.arguments());
  ...
  private void getActivityEffectiveArguments(final Context ctx) {
    try {
      ...
      final var arguments =
          this.missionModelService.getActivityEffectiveArguments(
              missionModelId, serializedActivity);
  ...
  private void extendExternalDataset(final Context ctx) {
    try {
      ...
      ctx.status(200)
          .result(Json.createObjectBuilder().add("datasetId", datasetId.id()).build().toString());
    } catch (final NoSuchPlanDatasetException ex) {
      ctx.status(404)
          .result(ResponseSerializers.serializeNoSuchPlanDatasetException(ex).toString());
    } catch (final InvalidJsonException ex) {
      ctx.status(400).result(ResponseSerializers.serializeInvalidJsonException(ex).toString());
    } catch (final InvalidEntityException ex) {
      ctx.status(400).result(ResponseSerializers.serializeInvalidEntityException(ex).toString());
