  ...
  public static final JsonParser<ProfileSegment<Optional<RealDynamics>>> realProfileSegmentP =
      productP
          ...
          .map(
              untuple(
                  (BiFunction<
                          Duration, Optional<RealDynamics>, ProfileSegment<Optional<RealDynamics>>>)
                      ProfileSegment::new),
              $ -> tuple($.extent(), $.dynamics()));

  public static final JsonParser<ProfileSegment<Optional<SerializedValue>>>
              ...
              .map(
                  untuple(
                      (BiFunction<
                              ...
                              ProfileSegment<Optional<SerializedValue>>>)
                          ProfileSegment::new),
                  $ -> tuple($.extent(), $.dynamics()));
          ...
          .map(
              profiles -> {
                final var realProfiles =
                    new HashMap<
                        String, Pair<ValueSchema, List<ProfileSegment<Optional<RealDynamics>>>>>();
                final var discreteProfiles =
                    new HashMap<
                        String,
                        Pair<ValueSchema, List<ProfileSegment<Optional<SerializedValue>>>>>();
                for (final var entry : profiles.entrySet()) {
                  ...
                  if (profile instanceof RealProfile p) {
                    ...
                    // If this happens, then the parser must have been updated without updating the
                    // mapping code
                    // It should not be possible to reach this point unless a new profile type is
                    // introduced and we
              ...
              profileSet -> {
                ...
                profileSet
                    ...
                    .forEach(
                        (name, profile) ->
                            profiles.put(
                                name, new RealProfile(profile.getLeft(), profile.getRight())));
                profileSet
                    .discreteProfiles()
                    .forEach(
                        (name, profile) ->
                            profiles.put(
                                name, new DiscreteProfile(profile.getLeft(), profile.getRight())));
                return profiles;
              });
}
