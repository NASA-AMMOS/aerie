...
/*package-local*/ final class ProfileRepository {
  static ProfileSet getProfiles(final Connection connection, final long datasetId)
      throws SQLException {
    final var realProfiles =
        new HashMap<String, Pair<ValueSchema, List<ProfileSegment<Optional<RealDynamics>>>>>();
    final var discreteProfiles =
        new HashMap<String, Pair<ValueSchema, List<ProfileSegment<Optional<SerializedValue>>>>>();
      ...
      switch (record.type().getLeft()) {
        case "real" -> realProfiles.put(
            ...
            Pair.of(
                ...
                getRealProfileSegments(
                    connection, record.datasetId(), record.id(), record.duration())));
        case "discrete" -> discreteProfiles.put(
            record.name(),
            Pair.of(
                record.type().getRight(),
                getDiscreteProfileSegments(
                    connection, record.datasetId(), record.id(), record.duration())));
  ...
  static List<PlanDatasetRecord> getAllPlanDatasetsForPlan(
      final Connection connection, final PlanId planId, final Timestamp planStartTime)
      throws SQLException {
  ...
  static List<ProfileRecord> getProfileRecords(final Connection connection, final long datasetId)
      throws SQLException {
  ...
  static void postResourceProfiles(
      final Connection connection, final long datasetId, final ProfileSet profileSet)
      throws SQLException {
    try (final var postProfilesAction = new PostProfilesAction(connection)) {
      final var profileRecords =
          postProfilesAction.apply(
              datasetId, profileSet.realProfiles(), profileSet.discreteProfiles());
      postProfileSegments(connection, datasetId, profileRecords, profileSet);
    }
  }
  ...
  static void appendResourceProfiles(
    ...
    try (final var getProfilesAction = new GetProfilesAction(connection)) {
      profileRecords =
          getProfilesAction.get(datasetId).stream()
    ...
    try (final var postProfilesAction = new PostProfilesAction(connection)) {
      final var newProfileRecords =
          postProfilesAction.apply(
              datasetId, newProfiles.realProfiles(), newProfiles.discreteProfiles());
      postProfileSegments(connection, datasetId, newProfileRecords, profileSet);
    }

    appendProfileSegments(connection, datasetId, profileRecords, oldProfiles);
  }
    ...
    for (final var entry : records.entrySet()) {
      ...
      switch (record.type().getLeft()) {
        case "real" -> postRealProfileSegments(
            connection, datasetId, record, realProfiles.get(resource).getRight());
        case "discrete" -> postDiscreteProfileSegments(
            connection, datasetId, record, discreteProfiles.get(resource).getRight());
  ...
  private static void appendProfileSegments(
    ...
    for (final var entry : profileSet.realProfiles().entrySet()) {
      ...
      try (final var appendProfileSegmentsAction = new AppendProfileSegmentsAction(connection);
        ...
        final var newProfileDuration =
            appendProfileSegmentsAction.apply(
                datasetId, record, realProfiles.get(resource).getRight(), realDynamicsP);
    ...
    for (final var entry : profileSet.discreteProfiles().entrySet()) {
      ...
      try (final var appendProfileSegmentsAction = new AppendProfileSegmentsAction(connection);
        ...
        final var newProfileDuration =
            appendProfileSegmentsAction.apply(
                datasetId, record, discreteProfiles.get(resource).getRight(), serializedValueP);
        updateProfileDurationAction.apply(datasetId, record.id(), newProfileDuration);
      }
    }
  }
