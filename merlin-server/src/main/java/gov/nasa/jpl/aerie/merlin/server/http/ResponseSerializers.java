...
public final class ResponseSerializers {
  public static <T> JsonValue serializeNullable(
      final Function<T, JsonValue> serializer, final T value) {
  ...
  public static JsonValue serializeParameters(final List<Parameter> parameters) {
    final var parameterMap =
        IntStream.range(0, parameters.size())
            ...
            .collect(
                Collectors.toMap(
                    i -> parameters.get(i).name(), i -> Pair.of(i, parameters.get(i))));
  ...
  public static JsonValue serializeValueSchemas(final Map<String, ValueSchema> schemas) {
    ...
    schemas.forEach(
        (k, v) ->
            builder.add(
                Json.createObjectBuilder().add("name", k).add("schema", serializeValueSchema(v))));
    return builder.build();
  }
  ...
  public static JsonValue serializeCreatedDatasetId(final long datasetId) {
    return Json.createObjectBuilder().add("datasetId", datasetId).build();
  }
    ...
    return Json.createObjectBuilder()
        .add(
            ...
            Json.createObjectBuilder()
                .add(
                    "activityInstanceIds",
                    serializeIterable(Json::createValue, violation.activityInstanceIds))
                .add("resourceIds", serializeIterable(Json::createValue, violation.resourceNames))
                .build())
        .add(
            "windows",
            serializeIterable(ResponseSerializers::serializeInterval, violation.violationWindows))
  ...
  private static JsonValue serializeSimulatedActivity(final SimulatedActivity simulatedActivity) {
    return Json.createObjectBuilder()
        ...
        .add(
            "parent",
            serializeNullable(id -> Json.createValue(id.id()), simulatedActivity.parentId()))
        .add(
            "children",
            serializeIterable((id -> Json.createValue(id.id())), simulatedActivity.childIds()))
        .add("computedAttributes", serializeArgument(simulatedActivity.computedAttributes()))
        .build();
  }

  private static JsonValue serializeSimulatedActivities(
      final Map<ActivityDirectiveId, SimulatedActivity> simulatedActivities) {
    return serializeMap(
  ...
  private static JsonValue serializeUnfinishedActivities(
      final Map<ActivityDirectiveId, UnfinishedActivity> simulatedActivities) {
    return serializeMap(
  ...
  private static JsonValue serializeUnconstructableActivityFailure(
      final MissionModelService.ActivityInstantiationFailure reason) {
    // TODO use pattern-matching switch expression here when available with LTS
    final var builder = Json.createObjectBuilder();
    if (reason
        instanceof final MissionModelService.ActivityInstantiationFailure.InstantiationFailure r) {
      return builder.add("reason", serializeInstantiationException(r.ex())).build();
    } else if (reason
        instanceof final MissionModelService.ActivityInstantiationFailure.NoSuchActivityType r) {
      return builder.add("reason", serializeNoSuchActivityTypeException(r.ex())).build();
    }
    throw new UnexpectedSubtypeError(
        MissionModelService.ActivityInstantiationFailure.class, reason);
  }

  public static JsonValue serializeUnconstructableActivityFailures(
      final Map<ActivityDirectiveId, MissionModelService.ActivityInstantiationFailure> failures) {
    if (failures.isEmpty()) {
      return Json.createObjectBuilder().add("success", JsonValue.TRUE).build();
    }
    return Json.createObjectBuilder()
        ...
        .add(
            ...
            serializeMap(
                ...
                failures.entrySet().stream()
                    .collect(
                        Collectors.toMap(
                            e -> Long.toString(e.getKey().id()), Map.Entry::getValue))))
        .build();
  }

  public static JsonValue serializeResourceSamples(
  ...
  public static JsonValue serializeConstraintViolations(
      final Map<String, List<Violation>> violations) {
    return Json.createObjectBuilder()
        .add(
  ...
  public static JsonValue serializeSimulationResultsResponse(
      final GetSimulationResultsAction.Response response) {
        ...
        .add(
            ...
            Json.createObjectBuilder()
                .add(
                    "extraneousArguments",
                    serializeStringList(
                        ex.extraneousArguments.stream().map(a -> a.parameterName()).toList()))
                .add(
                    "unconstructableArguments",
                    serializeIterable(
                        ResponseSerializers::serializeUnconstructableArgument,
                        ex.unconstructableArguments))
                .add(
                    "missingArguments",
                    serializeStringList(
                        ex.missingArguments.stream().map(a -> a.parameterName()).toList()))
                .build())
        .add(
            "arguments",
            serializeMap(
                ResponseSerializers::serializeArgument,
                ex.validArguments.stream()
                    .collect(
                        Collectors.toMap(
  ...
  public static JsonValue serializeJsonParsingException(final JsonParsingException ex) {
    ...
    return Json.createObjectBuilder().add("message", "invalid json").build();
  }
  ...
  public static JsonValue serializeMissionModelAccessException(
      final MissionModelAccessException ex) {
    // TODO: Improve diagnostic information?
    return Json.createObjectBuilder().add("message", ex.getMessage()).build();
  }
  ...
  public static JsonValue serializeNoSuchMissionModelException(
      final MissionModelService.NoSuchMissionModelException ex) {
    return Json.createObjectBuilder()
  ...
  public static JsonValue serializeNoSuchActivityTypeException(
      final MissionModelService.NoSuchActivityTypeException ex) {
    return Json.createObjectBuilder()
