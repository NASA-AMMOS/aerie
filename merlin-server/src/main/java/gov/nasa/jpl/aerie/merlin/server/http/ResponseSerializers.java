...
public final class ResponseSerializers {
  ...
  private static JsonValue serializeUnconstructableActivity(final SerializedActivity.Unconstructable unconstructable) {
    return Json
        .createObjectBuilder()
        .add("type", unconstructable.activity().getTypeName())
        .add("arguments", serializeArgumentMap(unconstructable.activity().getArguments()))
        .add("reason", unconstructable.reason())
        .build();
  }
