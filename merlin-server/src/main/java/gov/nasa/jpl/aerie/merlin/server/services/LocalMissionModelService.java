...
public final class LocalMissionModelService implements MissionModelService {
  ...
  public Map<String, ValueSchema> getResourceSchemas(final String missionModelId)
    ...
    // TODO: [AERIE-1516] Teardown the missionModel after use to release any system resources (e.g.
    // threads).
    final var schemas = new HashMap<String, ValueSchema>();

    for (final var entry :
        loadAndInstantiateMissionModel(missionModelId).getResources().entrySet()) {
  ...
  public List<ValidationNotice> validateActivityArguments(
    ...
    if (directiveType == null)
      return List.of(new ValidationNotice(List.of(), "unknown activity type"));
  ...
  public Map<ActivityDirectiveId, ActivityInstantiationFailure> validateActivityInstantiations(
    ...
    for (final var entry : activities.entrySet()) {
      ...
      try {
        ...
        final var specType =
            Optional.ofNullable(registry.directiveTypes().get(act.getTypeName()))
                .orElseThrow(
                    () -> new MissionModelService.NoSuchActivityTypeException(act.getTypeName()));
  ...
  public Map<String, SerializedValue> getActivityEffectiveArguments(
    ...
    final var modelType = this.loadMissionModelType(missionModelId);
    final var registry = DirectiveTypeRegistry.extract(modelType);
    final var directiveType =
        Optional.ofNullable(registry.directiveTypes().get(activity.getTypeName()))
            .orElseThrow(
                () -> new MissionModelService.NoSuchActivityTypeException(activity.getTypeName()));
  ...
  public SimulationResults runSimulation(final CreateSimulationMessage message)
    ...
    if (config.isEmpty()) {
      log.warn(
          "No mission model configuration defined for mission model. Simulations will receive an"
              + " empty set of configuration arguments.");
    }

    // TODO: [AERIE-1516] Teardown the mission model after use to release any system resources (e.g.
    // threads).
    return SimulationDriver.simulate(
        loadAndInstantiateMissionModel(
            message.missionModelId(), message.simulationStartTime(), SerializedValue.of(config)),
  ...
  public void refreshModelParameters(final String missionModelId)
      throws NoSuchMissionModelException {
    try {
      this.missionModelRepository.updateModelParameters(
          missionModelId, getModelParameters(missionModelId));
    } catch (final MissionModelRepository.NoSuchMissionModelException ex) {
      throw new NoSuchMissionModelException(missionModelId, ex);
    }
  }
  ...
  public void refreshActivityValidations(
    ...
    this.missionModelRepository.updateActivityDirectiveValidations(
        directive.id(), directive.planId(), directive.argumentsModifiedTime(), notices);
  }
  ...
  private ModelType<?, ?> loadMissionModelType(final String missionModelId)
    ...
    try {
      ...
      return MissionModelLoader.loadModelType(
  ...
  private MissionModel<?> loadAndInstantiateMissionModel(final String missionModelId)
    ...
    return loadAndInstantiateMissionModel(
        missionModelId, untruePlanStart, SerializedValue.of(Map.of()));
  }
  ...
  private MissionModel<?> loadAndInstantiateMissionModel(
      final String missionModelId, final Instant planStart, final SerializedValue configuration)
      throws NoSuchMissionModelException, MissionModelLoadException {
    try {
      final var missionModelJar = this.missionModelRepository.getMissionModel(missionModelId);
  ...
  public static class MissionModelLoadException extends RuntimeException {
    public MissionModelLoadException(final Throwable cause) {
      super(cause);
    }
  }
}
