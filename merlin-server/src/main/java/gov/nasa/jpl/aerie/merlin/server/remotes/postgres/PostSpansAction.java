...
/*package-local*/ final class PostSpansAction implements AutoCloseable {
  ...
  public Map<Long, Long> apply(
      final long datasetId, final Map<Long, SpanRecord> spans, final Timestamp simulationStart)
      throws SQLException {
    ...
    for (final var id : ids) {
      ...
      final var endTimestamp =
          act.duration()
              .map(
                  duration -> {
                    final var actEnd =
                        act.start()
                            .plus(duration.dividedBy(Duration.MICROSECOND), ChronoUnit.MICROS);
  ...
  private String buildAttributes(
    ...
    return activityAttributesP
        .unparse(new ActivityAttributesRecord(directiveId, arguments, returnValue))
        .toString();
  }
