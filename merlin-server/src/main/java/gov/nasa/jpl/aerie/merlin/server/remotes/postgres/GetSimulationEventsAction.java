...
/*package-local*/ final class GetSimulationEventsAction implements AutoCloseable {
  ...
  public SortedMap<Duration, List<EventGraph<Pair<Integer, SerializedValue>>>> get(
      final long datasetId, final Timestamp simulationStart) throws SQLException {
          ...
          transactions.forEach(
              ($, value) -> {
                try {
                  ...
                  throw new Error(
                      "Failed to unflatten EventGraph due to invalid tag at time point " + time, e);
                }
              });
        });
    return eventPoints;
  ...
  private static Map<
          Duration, SortedMap<Integer, List<Pair<String, Pair<Integer, SerializedValue>>>>>
      readResultSet(final ResultSet resultSet, final Timestamp simulationStart)
          throws SQLException {
    final var nodesByTimePoint =
        new HashMap<
            Duration, SortedMap<Integer, List<Pair<String, Pair<Integer, SerializedValue>>>>>();
    while (resultSet.next()) {
      ...
      nodesByTimePoint
          ...
          .add(Pair.of(causalTime, Pair.of(topicIndex, serializedValue)));
    }
  ...
  private static SerializedValue parseSerializedValue(final String value) {
    ...
    try (final var serializedValueReader = Json.createReader(new StringReader(value))) {
      serializedValue =
          serializedValueP.parse(serializedValueReader.readValue()).getSuccessOrThrow();
    }
