...
-- Commit merge needs to take all of the contents of the staging plan and apply them to the plan getting merged into
-- Then, it unlocks that plan for further edits
-- It then needs to update the relative merge request's status
create or replace procedure commit_merge(params)
  language plpgsql as $$
begin
  --update activity_directive(planid) to match merge_staging_area_activities for the plans activities
  --delete merge_staging_area_activities all entries associated with request_id
  --update merge_request id to accepted
  -- TODO
end
$$;
