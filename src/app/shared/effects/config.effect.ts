      ...
      this.http.get(action.url, { responseType: 'text' }).pipe(
        ...
        concatMap(projectConfig => {
            ...
            Object.keys(projectConfig).forEach(key => {
              if (key === 'itarMessage') {
                actions.push (new configActions.UpdateRavenSettings({ [key]: projectConfig[key]}));
              } else if (key === 'showTimeCursor' && projectConfig['showTimeCursor']) {
                actions.push (new timeCursorActions.ShowTimeCursor());
              } else {
                actions.push (new configActions.UpdateDefaultBandSettings({ [key]: projectConfig[key]}));
              }
            });
            return actions;
          }
