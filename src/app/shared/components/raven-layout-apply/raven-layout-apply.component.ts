...
export class RavenLayoutApplyComponent implements OnChanges {
  ...
  @Input() treeBySourceId: StringTMap<RavenSource>;

  @Output() applyLayout: EventEmitter<string[]> = new EventEmitter<string[]>();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.treeBySourceId) {
      ...
      this.filteredSources = getAllSourcesByKinds(tree, '/', ['fs_dir', 'fs_file', 'fs_graphable']).filter(
        ...
        (source: RavenSource) =>
          ...
          (tree[source.parentId].kind === 'fs_dir' && tree[source.parentId].expanded && tree[source.id].kind === 'fs_file') ||
          (tree[source.id].kind === 'fs_dir' && tree[source.id].childIds.reduce((include, childId) => include || tree[childId].kind === 'fs_graphable', false)),
      );
    }
  }
   ...
   */
  onApply() {
    this.applyLayout.emit(this.sourcesFormControl.value.map((value: RavenSource) => value.id));
