...
public class SchedulingDSLCompilationService {
  ...
  public SchedulingDSLCompilationService() throws IOException {
    ...
    this.nodeProcess =
        new ProcessBuilder(nodePath, "--experimental-vm-modules", schedulingDslCompilerCommand)
  ...
  public SchedulingDSLCompilationResult<SchedulingDSL.GoalSpecifier> compileSchedulingGoalDSL(
    ...
    try {
      ...
      return compile(
          missionModelTypes,
          goalTypescript,
          SchedulingDSL.schedulingJsonP(missionModelTypes),
          "Goal");
    } catch (IOException | MissionModelService.MissionModelServiceException e) {
      throw new Error(e);
    }
  ...
  private <T> SchedulingDSLCompilationResult<T> compile(
    ...
    final var schedulerGeneratedCode =
        TypescriptCodeGenerationService.generateTypescriptTypesFromMissionModel(missionModelTypes);
    final var constraintsGeneratedCode =
    ...
    try {
      ...
      return switch (status) {
        ...
        case "error" -> {
          ...
          try {
            yield new SchedulingDSLCompilationResult.Error<>(
                parseJson(output, SchedulingCompilationError.schedulingErrorJsonP));
          } catch (InvalidJsonException e) {
            throw new Error("Could not parse JSON returned from typescript: ", e);
          } catch (InvalidEntityException e) {
            throw new Error(
                "Could not parse JSON returned from typescript: " + e.failures + "\n" + output);
          }
        }
        case "success" -> {
          final var output = outputReader.readLine();
  ...
  public sealed interface SchedulingDSLCompilationResult<T> {
    ...
    record Error<T>(List<SchedulingCompilationError.UserCodeError> errors)
        implements SchedulingDSLCompilationResult<T> {}
  }
}
