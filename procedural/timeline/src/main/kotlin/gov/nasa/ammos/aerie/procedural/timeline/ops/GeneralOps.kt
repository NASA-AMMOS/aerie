...
interface GeneralOps<V: IntervalLike<V>, THIS: GeneralOps<V, THIS>>: Timeline<V, THIS> {
  ...
  fun <R: IntervalLike<R>, RESULT: GeneralOps<R, RESULT>> unsafeOperate(ctor: (Timeline<R, RESULT>) -> RESULT, f: Timeline<V, THIS>.(opts: CollectOptions) -> List<R>): RESULT {
    val result = gov.nasa.ammos.aerie.procedural.timeline.BaseTimeline(ctor) { f(it) }.specialize()
    return result.shouldCoalesce()?.let {
      gov.nasa.ammos.aerie.procedural.timeline.BaseTimeline(ctor) { opts ->
   ...
   */
  fun inspect(f: Consumer<List<V>>) = gov.nasa.ammos.aerie.procedural.timeline.BaseTimeline(ctor) {
      val list = collect(it)
      f.accept(list)
      list
  }.specialize()
  ...
  fun highlight(f: (V) -> Boolean) =
      unsafeMap(::Intervals, gov.nasa.ammos.aerie.procedural.timeline.BoundsTransformer.IDENTITY, false) {
        if (f(it)) it.interval
        else Interval.EMPTY
      }.convert(::Windows)

  /** [(DOC)][unsafeMap] **UNSAFE!** A simpler version of [unsafeMap] for operations that don't change the timeline type. */
  fun unsafeMap(boundsTransformer: gov.nasa.ammos.aerie.procedural.timeline.BoundsTransformer, truncate: Boolean, f: (V) -> V) = unsafeMap(ctor, boundsTransformer, truncate, f)
   ...
   */
  fun <R: IntervalLike<R>, RESULT: GeneralOps<R, RESULT>> unsafeMap(ctor: (Timeline<R, RESULT>) -> RESULT, boundsTransformer: gov.nasa.ammos.aerie.procedural.timeline.BoundsTransformer, truncate: Boolean, f: (V) -> R) =
   ...
   */
  fun <R: IntervalLike<R>, RESULT: GeneralOps<R, RESULT>> unsafeFlatMap(ctor: (Timeline<R, RESULT>) -> RESULT, boundsTransformer: gov.nasa.ammos.aerie.procedural.timeline.BoundsTransformer, truncate: Boolean, f: (V) -> Segment<GeneralOps<R, *>>) =
      unsafeOperate(ctor) { opts ->
   ...
   */
  fun unsafeMapIntervals(boundsTransformer: gov.nasa.ammos.aerie.procedural.timeline.BoundsTransformer, truncate: Boolean, f: (V) -> Interval) = unsafeMap(boundsTransformer, truncate) { v -> v.withNewInterval(f(v)) }
  ...
  fun shift(dur: Duration) = unsafeMapIntervals(gov.nasa.ammos.aerie.procedural.timeline.BoundsTransformer.shift(dur), false) { it.interval.shiftBy(dur) }
}
