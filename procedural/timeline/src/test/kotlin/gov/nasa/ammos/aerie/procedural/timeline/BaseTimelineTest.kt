...
class BaseTimelineTest {
  ...
  fun cache() {
    ...
    val profile = gov.nasa.ammos.aerie.procedural.timeline.BaseTimeline(::Constants) {
        collectCounter += 1
        listOf<Segment<Int>>()
    }
    ...
    profile.collect(gov.nasa.ammos.aerie.procedural.timeline.Interval.Companion.at(seconds(3)))
    assertEquals(collectCounter, 2)
