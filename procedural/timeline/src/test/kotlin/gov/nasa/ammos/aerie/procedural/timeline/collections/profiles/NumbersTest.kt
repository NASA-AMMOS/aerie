...
class NumbersTest {
  ...
  @Test
  fun plus() {
    val five = gov.nasa.ammos.aerie.procedural.timeline.collections.profiles.Numbers(
        Segment(
            between(Duration.ZERO, seconds(1)),
            5
        )
    ).assignGaps(gov.nasa.ammos.aerie.procedural.timeline.collections.profiles.Numbers(0))
  ...
  @Test
  fun increases() {
    val result = gov.nasa.ammos.aerie.procedural.timeline.collections.profiles.Numbers(
