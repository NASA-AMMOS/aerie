...
class GeneralOpsTest {
  ...
  fun map() {
    val result = Intervals(
        at(seconds(1)),
        seconds(2) .. seconds(3)
    ).unsafeMap(::Booleans, gov.nasa.ammos.aerie.procedural.timeline.BoundsTransformer.IDENTITY, false) { Segment(it.interval, it.interval.isPoint()) }
        .collect()
  ...
  fun flatMapTest() {
    val result = Intervals(
        ...
        .unsafeFlatMap(::Booleans, gov.nasa.ammos.aerie.procedural.timeline.BoundsTransformer.IDENTITY, false) {
